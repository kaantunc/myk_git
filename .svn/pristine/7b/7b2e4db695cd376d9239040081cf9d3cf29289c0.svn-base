...
class JTableUser extends JTable
{
	...
	*/
	...
	function check() 
	{
	    ...
	    if (trim( $this->name ) == '') {
			$errorMessage .= JText::_('Please enter your name.');
	        $this->setError( JText::_( $errorMessage ) );
	        return false;
		}
	     
		if (trim( $this->username) == '') {
	        $errorMessage .= JText::_('Please enter a user name.');
	        $this->setError( JText::_( $errorMessage ) );
	        return false;
		}
	      
		if (preg_match( "#[<>\"'%;()&]#i", $this->username) || strlen(utf8_decode($this->username )) < 2) {
			//$errorMessage .= JText::_('Please enter a username containing only letters, numbers, and no spaces.');
	      	JText::sprintf( 'VALID_AZ09', JText::_( 'Username' ), 2 );
			$this->setError( JText::_( $errorMessage ) );
			return false;
		}
	    ...
	    if ($xid && $xid != intval( $this->id )) {
	        $errorMessage .= JText::_('WARNREG_INUSE');
			$this->setError( JText::_( $errorMessage ) );
	        return false;
	    }
	
	    if ((trim($this->email) == "") || ! JMailHelper::isEmailAddress($this->email) ) {
			$errorMessage .= JText::_('WARNREG_MAIL');
	        $this->setError( JText::_( $errorMessage ) );
	        return false;
		}
	    ...
	    $xid = intval( $this->_db->loadResult() );
	    if ($xid && $xid != intval( $this->id ) ) {
			$errorMessage .= JText::_('WARNREG_EMAIL_INUSE');
	      	$this->setError( JText::_( $errorMessage ) );
	        return false;
		}
	    ...
	    // Add password verification error messages
	    $errorMessage .= $_SESSION['password_check'];
	    unset($_SESSION['password_check']);
	    	         
	    // Terms of Service
//	    if (trim( $this->terms) == '') {
//	    	$errorMessage .= '<li>You must read and agree to the terms of service to register.</li>';
//	    }
	      
	    // Set system message equal to error messages
	    $this->setError( JText::_( $errorMessage ) );
