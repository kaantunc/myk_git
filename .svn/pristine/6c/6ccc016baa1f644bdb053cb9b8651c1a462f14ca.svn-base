...
<div class="anaDiv font18">
	...
	<div class="div80 fontBold">
		...
		$YetUcret = $BelgeUcretBilgi[$itiraz[0]['BELGENO']]['YetUcret'];
		
		if(UcretHesapla($hesap) > UcretHesapla($YetUcret[$itiraz[0]['BELGENO']]['UCRET'])){
			$hesap = UcretHesapla($YetUcret[$itiraz[0]['BELGENO']]['UCRET']);
		}
...
<div class="anaDiv font18">
	...
	<div class="div80 fontBold">
		...
		if($dosyaType == 'pdf' || $dosyaType == 'doc' || $dosyaType == 'docx' || $dosyaType == 'zip' || $dosyaType == 'rar'){
			echo '<td><a target="_blank" href="index.php?dl=sinavTesvikItiraz/'.$itiraz[0]['SINAV_ID'].'/'.$itiraz[0]['ITIRAZ_DOSYA'].'" class="btn btn-sm btn-info">İndir</a></td>';
		}else if($dosyaType == 'jpg' || $dosyaType == 'jpeg' || $dosyaType == 'png' || $dosyaType == 'gif' || $dosyaType == 'pjpeg'){
			echo '<td><a target="_blank" href="index.php?img=sinavTesvikItiraz/'.$itiraz[0]['SINAV_ID'].'/'.$itiraz[0]['ITIRAZ_DOSYA'].'" class="btn btn-sm btn-info">İndir</a></td>';
		}
		?>
	</div>
</div>
...
<div class="anaDiv">
	<div class="div60">
		<a href="index.php?option=com_tesvik&view=tesvik&layout=itirazlar" class="btn btn-sm btn-danger">Geri</a>
	</div>
	...
	if(durum == 1){
		if(confirm('Onaylanan veya Reddedilen itirazlar üzerinde tekrardan işlem yapılamaz. İtiraz ücretini kabul ederek, onaylamak istediğinizden emin misiniz?')){
			jQuery.ajax({
				async:false,
				type:"POST",
				url:"index.php?option=com_tesvik&task=AjaxItirazDurumGuncelleWithBelgeNo&format=raw",
				data:'belgeNo='+belgeNo+'&durum='+durum
			}).done(function(data){
				var dat = jQuery.parseJSON(data);
				if(dat){
					alert('İtiraz Onaylandı.');
					window.location.href = "index.php?option=com_tesvik&view=tesvik&layout=itirazlar";
				}else{
...
function FuncItirazUcretDuzenle(belgeNo){
	...
	jQuery.ajax({
		async:false,
		type:"POST",
		url:"index.php?option=com_tesvik&task=AjaxItirazWithBelgeNo&format=raw",
...
function FuncItirazUcretKaydet(belgeNo){
	if(confirm('İtiraz Ücretini Güncellemek İstediğinizden Emin misiniz?')){
		...
		jQuery.ajax({
			async:false,
			type:"POST",
			url:"index.php?option=com_tesvik&task=AjaxItirazUcretKaydetWithBelgeNo&format=raw",
