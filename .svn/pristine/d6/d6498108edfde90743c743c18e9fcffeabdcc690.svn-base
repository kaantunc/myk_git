...
echo 'action=index.php?option=com_yeterlilik_taslak&layout=kaynak&'.$task.'&yeterlilik_id='.$this->yeterlilik_id;	
?>
...
<div class="form_item">
  <div class="form_element cf_heading">
  	<h1 class="contentheading">YETERLİLİĞE KAYNAK TEŞKİL EDEN YETERLİLİK BİRİMLERİ</h1>
  </div>
  <div class="cfclear">&nbsp;</div>
</div>
...
?>
	<div class="form_item" style="padding-top: 25px;">
		<div class="form_element cf_button">
			<input value="Yorum Kaydet" name="kaydet" type="button"  onclick="yorumKaydet('kaynak', <?php echo $this->yeterlilik_id;?>)"/>
		</div>
		<div class="cfclear">&nbsp;</div>
	</div>
<?php 
...
function getStandart(val, id){
	...
	jQuery.ajax({  
		url: "index.php?option=com_yeterlilik_taslak&task=getStandart&format=raw&standartTur=" + val,  
		dataType: 'json',    
		async: false,  
