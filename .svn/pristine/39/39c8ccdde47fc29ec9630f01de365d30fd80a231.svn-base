<?php
	...
	foreach($allfieldsnames as $allfieldsname){
		if(!isset($post[$allfieldsname]) && $session->get($allfieldsname)=== false){
			$post[$allfieldsname] = '';
		}else{
			if (isset($post[$allfieldsname])){
				...
				if(!is_array($session->get($allfieldsname))){
					$post[$allfieldsname] = htmlentities($session->get($allfieldsname), ENT_QUOTES, 'UTF-8');
				}
			}
		}
	}
