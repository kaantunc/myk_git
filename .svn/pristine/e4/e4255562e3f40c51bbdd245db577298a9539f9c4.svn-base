...
class Protokol_YetModelProtokol_Yet extends JModel {
	...
	/*OK*/function getProtokoller()
	{
		...
		$params = array (PM_PROTOKOLTURU_YETERLILIK);
		return $db->prep_exec($sql, $params);
	
	}
	
	/*OK*/function getEtkinlikDurumlari()
	{
		$db  = &JFactory::getOracleDBO();
	...
	/*OK*/function getYetkiliKuruluslar (){
		$db  = &JFactory::getOracleDBO();
		$sql = "SELECT 	USER_ID,
						KURULUS_ADI,
						KURULUS_YETKILISI,
						...
						KURULUS_SEHIR  
					FROM m_kurulus    
					WHERE kurulus_durum_id IN (".YETKILI_YET_KURULUS_DURUMLARI.")
					ORDER BY USER_ID";
	
		return $db->prep_exec($sql, array());
	}
	
	/*OK*/function getUzatmalar($protokolID)
	{
		$db  = &JFactory::getOracleDBO();
		$sql = "SELECT *
	...
	/*OK*/function yetProtokoluMu($protokolID)
	{
		$result = false;
		$db  = &JFactory::getOracleDBO();
		$sql = "SELECT * FROM m_protokol WHERE protokol_turu = ? AND protokol_id = ?";
	
		$params = array ( PM_PROTOKOLTURU_YETERLILIK ,$protokolID);
