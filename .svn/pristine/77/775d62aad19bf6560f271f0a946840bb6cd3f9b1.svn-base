...
//merkezGeriAl Bas
function merkezGeriAl(value, merkezid, yetId){
	var id = 'sinav_yeri';
	var selectBox = document.getElementById(id);
	addOption(selectBox, value, merkezid, yetId, 'merkezSecildi');
}
//merkezGeriAl Son
...
jQuery(".denetle").live("click",function(e){
	e.preventDefault();
	var valYet = jQuery("#yeterlilik_konusu").val();
	var valMerkez = jQuery("#sinav_yeri").val();
	var valTar = jQuery("#sinav_tarihi").val();
	var valSaat = jQuery("#inputsinav_saati").val();
	
	var sendData = 'yetId=' + valYet + '&merkez_id='+valMerkez + '&tarih=' + valTar + '&saat=' + valSaat + '&user_id=' + user_id;
	var url = 'index.php?option=com_sinav&task=sinavOncedenVarmi&format=raw';
    	jQuery.ajax({
    		  url: url,
    		  data: sendData,
    		  type: "POST",
    		  ...
    		  success: function(data) {
    			  if(data['success']){
    				  alert("Aynı yeterliğe, aynı yere, aynı tarihe ve aynı saate göre sınavınız mevcut. Lütfen eski sınavınızı güncelleyin veya yeni bir sınav oluşturun.");
    				  return false;
     			  }else{
     				 jQuery("#sinav_oncesi_form").submit();
    			  }
    		  }
    	});	
...
<div class="form_item">
  <div class="form_element cf_button">
    <input class="denetle" value="İleri" name="submitButton" type="button"/>
  </div>
