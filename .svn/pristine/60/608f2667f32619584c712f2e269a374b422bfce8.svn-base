...
var ArtLay	=
{
	...
	category : function(dbc)
	{
		...
		this.layout._genericLayoutDelayed({
			...
			// Toolbar settings
			toolbar : {
				back : this.layout.section.bind(this, [c.section.toInt()]),
				config : true,
				insert : this.insert.bind(this)
			},
	...
	article : function(dba)
	{
		...
		if ($type(dba) == 'number')
		{
			// Build SQL query
			var q	=
			'SELECT a.id, a.title, a.introtext AS description, a.catid,'+
		...
		if (a.catid > 0)
			bck	= this.layout.category.bind(this, a.catid.toInt());
