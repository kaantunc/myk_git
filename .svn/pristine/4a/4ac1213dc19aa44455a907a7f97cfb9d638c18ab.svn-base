...
?>
<form
	action=index.php?option=com_yeterlilik_taslak_yeni&view=taslak_revizyon&task=revizyonKaydet&yeterlilik_id=<?php echo $this->yeterlilik_id;?>	
...
<input type=button value="Git" onclick="git(jQuery('#revizyon_no').val());"></label>
		<?php 
		if ($_GET[revizyon]!="yeni" and $durumkontrol[0]==1 and $durumkontrol[1]!=""){
		?>
<input type="button" <?php echo $disabled;?> value="Yeni Revizyon Oluştur" onclick="window.location='index.php?option=com_yeterlilik_taslak_yeni&view=taslak_revizyon&yeterlilik_id=<?php echo $_GET[yeterlilik_id];?>&revizyon=yeni'">
		<?php			
		} else { 
	?>
		<input type="button" value="Yeni Revizyon Oluştur" disabled title="Yeterlilik veya Revizyon durumu 'Yönetim Kurulu Onayladı' ise ve yeterliliğin Son Hali PDF olarak yüklenmişse buton aktif olur.">
	<?php 					
		}
		?>
...
<?php 
	...
	<div class="form_item">
	  <div class="form_element cf_textbox">
	    <label class="cf_label" style="width: 150px;font-weight:bold;">Belge Geçerlilik Süresi:</label>
	    <input class="cf_inputbox" maxlength="150" size="20" <?php echo $this->disabled;?>
	    	id="gecerlilik" name="gecerlilik" type="text" value="<?php echo $yet_bilgi['GECERLILIK_SURESI'];?>" />
	  </div>
	  <div class="cfclear">&nbsp;</div>
	</div>
<?php 
...
function git(id){
	window.location='index.php?option=com_yeterlilik_taslak_yeni&view=taslak_revizyon&yeterlilik_id=<?php echo $_GET[yeterlilik_id];?>&revize_no='+id;
}
...
function addTaslakPdf (tableName){
	...
	for (var i = 0; i < 4; i++){//6
		if (paths[i] != null && paths[i] != ''){
			...
			var result = inputPath + '<br><div id="success_'+tableName+'_0_'+sira+'" class="up_success">'+fileNames[i]+' yüklendi!</div><div> <input type="button" value="İndir / Oku" onclick="window.location.href=\'index.php?option=com_yeterlilik_taslak_yeni&amp;view=taslak_revizyon&amp;task=indir&amp;id='+sira+'&amp;yeterlilik_id=<?php echo $this->yeterlilik_id;?>&amp;revize_no=<?php echo $_GET[revize_no];?>\'" class="up_submitbtn" style="float:none;"><input type="button" value="Sil / Değiştir" onclick="window.location.href=\'index.php?option=com_yeterlilik_taslak_yeni&amp;view=taslak_revizyon&amp;task=sil&amp;id='+sira+'&amp;yeterlilik_id=<?php echo $this->yeterlilik_id;?>&amp;revize_no=<?php echo $revize_no;?>\'" class="up_submitbtn" style="float:none;"></div>';
...
function updateDisabledInputs (durumId){
	...
	var bekletiliyorId = <?php echo YONETIM_KURULUNDA_BEKLETILIYOR?>;
	...
	var sektorSorumlusuDuzenlemeYetkiId=20;

	for (var i = 1; i < 4; i++){
		document.getElementById ("taslakPdf_0_file_" + i).className = "";
	}
	if (durumId == -2 || durumId == 0 || durumId == 3 || durumId == 10 ){
	...
	if (durumId == onaylanmisId || durumId == bekletiliyorId || durumId == sektorSorumlusuDuzenlemeYetkiId){
...
//	var resmiGazeteId = <?php echo RESMI_GAZETEDE_YAYINLANMIS_STANDART?>;
	...
	else if (rgdosya.length == 0 && (gId1 == durumId || gId2 == durumId || skId1 == durumId || skId2 == durumId || skId3 == durumId || ykId1 == durumId || ykId2 == durumId || ykId3 == durumId || ykIdson == durumId)){
		alert ("Resmi görüşe/Kamuoyuna sunmadan önceki taslağı yüklemelisiniz.");
		return false;
	}
	else if (skdosya.length == 0 && (skId1 == durumId || skId2 == durumId || skId3 == durumId || ykId1 == durumId || ykId2 == durumId || ykId3 == durumId || ykIdson == durumId)){
