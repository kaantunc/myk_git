...
if (!class_exists('TCPDF')) {
	 ...
	 */ 
	define('PDF_PRODUCER', 'TCPDF 4.8.020_PHP4 (http://www.tcpdf.org)');
	...
	*/
	class TCPDF {
		...
		/**
		* @var headerin sağ yanındaki text
		* @access protected
		*/
		var $headerTexti;
		
		/**
		 * @var headerin sol yanındaki text
		 * @access protected
		 */
		var $headerTextiSol;
		...
		*/
		function AddPage($orientation='', $format='',$tempPage) {
			...
			$this->endPage();
			// start new page
			$this->startPage($orientation, $format,$tempPage);
		}
		...
		* @see endPage(), AddPage()
		*/
		function startPage($orientation='', $format='',$tempPage) {
		...
		/**
	 	 ...
	 	 * @since 4.0.012 (2008-07-24)
		 */
		function setHeader($tempPage=true) {
		...
		*/
		function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign=0) {	
			...
			if($valign) { 
				$this->y -= $this->cMargin;
				$numLines = $this->getNumLines($txt, $w);
				$cellratio = $this->getCellHeightRatio();
				$fontsize = $this->GetFontSize();
				$contentheight = $numLines * $cellratio * $fontsize;
				$addy = (($h - $contentheight) / 2) ;
				$this->y += $addy;
			} 
		...
		*/
		 ...
		 */
		function getImageFileType($imgfile) {
			$type = ''; // default type
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		 ...
		 */
		function writeHTMLCell($w, $h, $x, $y, $html='', $border=0, $ln=0, $fill=0, $reseth=true, $align='', $autopadding=true, $valign = 0) {
			return $this->MultiCell($w, $h, $html, $border, $align, $fill, $ln, $x, $y, $reseth, 0, true, $autopadding, 0, $valign);
		}
