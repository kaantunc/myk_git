...
<table cellspacing="0" class="paginate-10 sortable">
	...
	<?php
		...
		foreach($sonuclar AS $satir){
			...
			echo '<td><a href="?option=com_yeterlilik_taslak_yeni&view=gorus_ayrinti&gorusId='.$satir['GORUS_ID'].'&yeterlilikId='.$this->yeterlilikId.'">Ayrıntılar</a></td>';
			echo '</tr>';
			$rowCount++;
		}
	?>
...
<br />
<br />
<a target="_blank" href="index.php?option=com_yeterlilik_taslak_yeni&view=gorus_listele&layout=pdf&format=pdf&yeterlilikId=<?php echo $this->yeterlilikId;?>">PDF Çıktısı Al</a>
