...
$data = $this->pm_il; 
echo $this->pageTree;
if ($this->canEdit || $this->kurulus['BASVURU_DURUM'] == 0 || empty($this->kurulus)){
	$action='index.php?option=com_uzman_basvur&amp;layout=uzman_bilgi&amp;task=basvuruKaydet';
?>
...
if (!$this->canEdit && ($this->kurulus['UZMAN'] != 0 || $this->kurulus['DENETCI'] != 0)) {
	$disabled='disabled = "disabled"';
} else {
	echo '<input type="hidden" name="user_id" value="'.$this->kurulus["USER_ID"].'" />';
}
...
<div class="anaDiv">
	...
	<div class="div80 font16">
		<input class="cf_inputbox input-sm" <?php echo $readonly;?> maxlength="150" size="30"  id="tckimlik" name="tckimlik" type="text" value="<?php echo $this->kurulus["TC_KIMLIK"];?>" <?php echo $disabled;?>/><b>&nbsp;&nbsp;*</b>
	</div>
</div>
...
?>	
	<div class="form_item" style="padding-top: 25px;">
		<div class="form_element cf_button">
			<button name="islem" value="olustur" type="submit" type="submit"  class="styled-button-8" onclick="return kontrol();"/>Kaydet</button>
		</div>
		<div class="cfclear">&nbsp;</div>
	</div>
<?php 
...
jQuery(document).ready(function(){
	...
	jQuery('#fotoIptal').live('click',function(e){
		...
		function kontrol(){
			...
			if(jQuery('input[name="kurum"]').val() == ""){
			...
			if(jQuery('textarea[name="adres"]').val() == ""){
