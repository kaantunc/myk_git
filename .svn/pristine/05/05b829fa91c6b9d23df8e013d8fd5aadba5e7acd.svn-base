...
?>	
	...
	<table border="1">
		...
		echo tableRow ($count++, "SEVİYESİ:", $yeterlilikBilgi["SEVIYE_ADI"]);
		...
		if ($kaynakMeslek != null){
			foreach ($kaynakMeslek as $row){
				if ($row["KAYNAK_ID"] == -1)
					...
					$kaynakMeslekHTML .=  statikTabloHTML ("", getStandartAdi($row["KAYNAK_ID"]));
...
<?php 
		...
		echo tableRow ($count++, "YETERLİLİĞİ GELİŞTİREN KURULUŞ(LAR)", $katkiKurulusHTML);
