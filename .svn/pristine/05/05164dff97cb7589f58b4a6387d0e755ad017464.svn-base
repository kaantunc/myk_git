...
function getTakvimUpdate(str){
	...
	for(var i=0;i<rows.length;i++){
		...
		inp.setAttribute("onclick", "tumYeterlilikAl('"+ merkezInp.id +"', '"+merkezInp.value+"')");
...
function tumYeterlilikAl(merkezId, merkezVal){

	var yetInpNo = merkezId.substring(merkezId.lastIndexOf('-')+1);

	var yetInpId = 'inputsinavTakvimi-4-' + yetInpNo;

	var yetInp = document.getElementById(yetInpId);
	
	merkezSecildi(merkezId, merkezVal);
