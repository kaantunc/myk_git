...
<table width="100%" border="1" cellpadding="0" cellspacing="2" id="kurTable" class="display compact">
		...
		foreach ($itirazlar as $row){
			...
			if($hesap > UcretDuzenle($YetUcret[$row['BELGENO']]['UCRET'])){
				$hesap = UcretDuzenle($YetUcret[$row['BELGENO']]['UCRET']);
			}
			...
			if($this->dId == 0){
				echo '<td><a href="index.php?option=com_tesvik&view=tesvik&layout=itiraz_belgeno&belgeno='.urlencode($row['BELGENO']).'" class="btn btn-xs btn-default">Düzenle</a></td>';
			}
