...
echo 'action=index.php?option=com_yeterlilik_taslak&layout=ek_kurulus&'.$task.'&yeterlilik_id='.$this->yeterlilik_id;	
?> 
...
<div class="form_item">
  <div class="form_element cf_heading">
  	<h2 class="contentheading">EK 5 : Yeterlilik Taslağının Görüşe Gönderildiği Kurum ve Kuruluşlar</h2>
  </div>
  <div class="cfclear">&nbsp;</div>
</div>
...
?>
	<div class="form_item" style="padding-top: 25px;">
		<div class="form_element cf_button">
			<input value="Yorum Kaydet" name="kaydet" type="button"  onclick="yorumKaydet('ek_kurulus', <?php echo $this->yeterlilik_id;?>)"/>
		</div>
		<div class="cfclear">&nbsp;</div>
	</div>
<?php 
...
function addKurulusValues (kurulus, name){
	...
	<?php
	$tableCount = count ($this->gorus_kurulus);
	
	$c = 0;
	$id = 0;
	for ($i=0; $i< $tableCount; $i++) {
		$arr = $this->gorus_kurulus[$i];
