...
if(file_exists('libraries'.DS.'form'.DS.'form_config.php'))
...
class JUser extends JObject
{
	...
	*/
	...
	function bind(& $array)
	{
		...
		if (empty($this->id))
		{
			...
			if (empty($array['password'])) {
				...
				$_SESSION['password_check'] = JText::_('Please create a password for your account.');
			}
			if ($array['password'] != $array['password2']) {
            //	$this->setError( JText::_( 'PASSWORD DO NOT MATCH.' ) );
            //  return false;
				$_SESSION['password_check'] .= JText::_( 'PASSWORDS_DO_NOT_MATCH' );
			}
	...
	function save( $updateOnly = false )
	{
		...
		if ($_POST[active]==3){
			mysql_query("update jos_users set active=".$_POST[active]." where id=".$table->id);
		}
	...
	function getOracleUserId (){
		$db = & JFactory::getDBO();
		
		$sql = "SELECT tgUserId FROM #__users WHERE id = ".$this->id;
		$db->setQuery($sql);
		$data = $db->loadRow();
	...
	function getAkreditasyonId(){
		$oraDb = & JFactory::getOracleDBO();
		
		$tgUserId = $this->getOracleUserId();
		
		$sql = "SELECT DISTINCT AKREDITASYON_ID
			FROM M_AKREDITASYON
			NATURAL JOIN M_BASVURU
				WHERE USER_ID = ?";
		
		$rows = $oraDb->prep_exec($sql, array($tgUserId));
		
		if(isset($rows[0]['AKREDITASYON_ID']))
			return $rows[0]['AKREDITASYON_ID'];
		else
			return false;
	...
	function getActive (){
		...
		$sql = "SELECT active FROM #__users WHERE id = ".$this->id;
		$db->setQuery($sql);
		$data = $db->loadRow();
	...
	function evrakSahibiMi($evrakId){
		$db = & JFactory::getOracleDBO();
		
		$tgUserId = $this->getOracleUserId();
		
		$sql = "SELECT USER_ID FROM EVRAK WHERE EVRAK_ID = ?";
		
		$rows = $db->prep_exec($sql, array($evrakId));
		
		if(isset($rows[0]['USER_ID']) && isset($rows[0]['USER_ID']) == $tgUserId)
			return true;
		else
			return false;
	...
	function yeterlilikSahibiMi($yetId){
		$db = & JFactory::getOracleDBO();
		...
		$sql = "SELECT USER_ID
					FROM EVRAK
					NATURAL JOIN M_YETERLILIK_TALEBI
