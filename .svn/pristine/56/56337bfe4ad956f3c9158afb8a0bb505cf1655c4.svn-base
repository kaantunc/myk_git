...
class SinavModelSinav_Oncesi_Ozet extends JModel {
	...
	function getKurulus($db, $user_id){
		$sql = "SELECT KURULUS_ADI
							FROM M_KURULUS
						WHERE USER_ID = ?";
		$yetkiNo = $db->prep_exec($sql,array($user_id));
		if(!empty($yetkiNo))
		return $yetkiNo[0]["KURULUS_ADI"];
		else
		return false;
    ...
    function getYetAdi($db, $postData){
		...
		$sql = "SELECT YETERLILIK_ADI, YETERLILIK_KODU
					FROM M_YETERLILIK
				WHERE YETERLILIK_ID = ?";
		$yetkiNo = $db->prep_exec($sql,array($postData['yeterlilik_konusu']));
			...
			if(!empty($yetkiNo)){
				for($jj = 0; $jj<count($yetkiNo); $jj++){
					if($sekilliste[$jj] == 0)
						...
						//$sonuc .= $birimler[$jj].'	-	'.$yetkiNo[$jj][0]['YETERLILIK_ALT_BIRIM_ADI'].'	(Teorik)<br>';
					else
					$sonuc .= $yetkiNo[$jj][0]['YETERLILIK_ALT_BIRIM_NO'].'	-	'.$yetkiNo[$jj][0]['YETERLILIK_ALT_BIRIM_ADI'].'	(Pratik)<br>';
					//$sonuc .= $birimler[$jj].'	-	'.$yetkiNo[$jj][0]['YETERLILIK_ALT_BIRIM_ADI'].'	(Pratik)<br>';
				}
				return $sonuc;
			}
			else
	...
	function getPdfSekilAdi($db, $postData){
		...
		if($sekilliste[0] == null){
			...
			if(!empty($yetkiNo)){
				for($jj = 0; $jj<count($yetkiNo); $jj++){
					//$sonuc .= '<div style ="width:100%; float:left;">'.$yetkiNo[$jj][0]['ID'].'	-	'.$yetkiNo[$jj][0]['BIRIM_ADI'].$yetkiNo[$jj][0]['BIRIM_KODU'].$yetkiNo[$jj][0]['OLC_DEG_HARF'].$yetkiNo[$jj][0]['OLC_DEG_NUMARA'].'</div>';
					$sonuc .= '<div style ="width:100%; float:left;">'.$yetkiNo[$jj][0]['BIRIM_ADI'].$yetkiNo[$jj][0]['BIRIM_KODU'].$yetkiNo[$jj][0]['OLC_DEG_HARF'].$yetkiNo[$jj][0]['OLC_DEG_NUMARA'].'</div>';
				}
				return $sonuc;
			}
			else
			...
			if(!empty($yetkiNo)){
				for($jj = 0; $jj<count($yetkiNo); $jj++){
					if($sekilliste[$jj] == 0)
						...
						$sonuc .= '<div style ="width:100%; float:left;">'.$yetkiNo[$jj][0]['YETERLILIK_ALT_BIRIM_ADI'].'	(Teorik)</div>';
					else
						//$sonuc .= '<div style ="width:100%; float:left;">'.$birimler[$jj].'	-	'.$yetkiNo[$jj][0]['YETERLILIK_ALT_BIRIM_ADI'].'	(Pratik)</div>';
					$sonuc .= '<div style ="width:100%; float:left;">'.$yetkiNo[$jj][0]['YETERLILIK_ALT_BIRIM_ADI'].'	(Pratik)</div>';
				}
				return $sonuc;
			}
			else
