...
{
	var abulu = $ES('input', $(chronoformname));
	...
	$ES('input', $(chronoformname)).each(function(field){
		...
		if ( ['text', 'password', 'file'].contains(type) ) {
			...
			if(field.getProperty('title')){
				tmessage = field.getProperty('title').split(",");
			}
			
			if(field.hasClass('required')){
				if( tmessage[0] ) { var message_required = tmessage[0]; }
		...
		if ( ['text', 'password'].contains(type) ) {	
			if(field.hasClass('validate-number')){
				if( tmessage[1]) { var message_validate_number = tmessage[1]; }
			...
			if(field.hasClass('validate-digits')){
				if( tmessage[2]) { var message_validate_digits = tmessage[2]; }
				name.add( Validate.Numericality, { 
			...
			if(field.hasClass('validate-alpha')){
				if( tmessage[3]) { var message_validate_alpha = tmessage[3]; }
				name.add( Validate.Format, { 
					pattern: /^[a-zA-ZçöğüşıÇÖĞÜŞİ ]+$/, 
			...
			if(field.hasClass('validate-alphanum')){
				if( tmessage[4]) { var message_validate_alphanum = tmessage[4]; }
				name.add( Validate.Format, { 
					pattern: /^[0-9a-zA-ZçöğüşıÇÖĞÜŞİ ]+$/, 
			...
			if(field.hasClass('validate-date')){
				if( tmessage[5]) { var message_validate_date = tmessage[5]; }
				name.add( Validate.Format, { 
			...
			if(field.hasClass('validate-email')){
				if( tmessage[6]) { var message_validate_email = tmessage[6]; }
			...
			if(field.hasClass('validate-url')){
				if( tmessage[7]) { var message_validate_url = tmessage[7]; }
				name.add( Validate.Format, {
			...
			if(field.hasClass('validate-date-au')){
				if( tmessage[8]) { var message_validate_date_au = tmessage[8]; }
				name.add( Validate.Format, {
			...
			if(field.hasClass('validate-currency-dollar')){
				if( tmessage[9]) { var message_validate_currency_dollar = tmessage[9]; }
				name.add( Validate.Format, {
		...
		if ( ['checkbox', 'radio'].contains(type) ) {
			...
			if(field.hasClass('required')){
				if( tmessage[10] ) { var message_required = tmessage[10]; }
				fieldsarray[fieldsarray_count] = name;
				fieldsarray_count = fieldsarray_count + 1;
				name.add( Validate.Presence, { 
					failureMessage: message_required 
			...
			if(field.hasClass('validate-one-required')){
				if( tmessage[11]) { var message_validate_one_required = tmessage[11]; }
	...
	$ES('select', $(chronoformname)).each(function(field){
		...
		if(field.hasClass('validate-selection')){
			if( tmessage[12] ) { var message_validate_selection = tmessage[12]; }
	...
	$ES('textarea', $(chronoformname)).each(function(field){
		...
		if(field.hasClass('required')){
			if( tmessage[13] ) { var message_required = tmessage[13]; }
			//if( field.size > 1 ) { field.options[0].selected = true; }
			name.add( Validate.Presence, { 
				failureMessage: message_required
			});
