		...
		elseif (!isset($GLOBALS['_JREQUEST'][$name][$sig]))
		{
			if (isset($input[$name]) && $input[$name] !== null) {
				...
				if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
					...
					$var = JRequest::_CleanStrip_tags( $var );
					$var = JRequest::_CleanSqlInjection( $var );
					$var = JRequest::_CleanHtmlspecialchars( $var );
				}
...
;				$mainframe->redirect($return, JText::_('SESSION_EXPIRED'));
	...
	function _CleanHtmlspecialchars($value){
		$value = is_array( $value ) ? array_map( array( 'JRequest', '_CleanHtmlspecialchars' ), $value ) : htmlspecialchars( $value,ENT_QUOTES,'UTF-8' );
		return $value;
	}
	
	function _CleanStrip_tags( $value ){
		$value = is_array( $value ) ? array_map( array( 'JRequest', '_CleanStrip_tags' ), $value ) : strip_tags( $value );
		return $value;
	}
	
	function _CleanSqlInjection($value){
		$sqlInjection = array('/insert/i','/update/i','/delete/i','/union/i','/select/i','/drop/i','/exec/i');
		$value = is_array( $value ) ? array_map( array( 'JRequest', '_CleanSqlInjection' ), $value ) : preg_replace($sqlInjection, '', $value );
		return $value;
	}
}
