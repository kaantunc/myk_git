...
function patchSatirEkleWithDatePick(id, headers, columnNoArr, dTabloIndex, counterId){
	counterId = counterId == null ? id : counterId;
	...
	ekleInp.onclick = function(){
		addNRow(id , headers.length , dTabloIndex);
...
function patchSatirEkle(id, headers, dTabloIndex, counterId){
	counterId = counterId == null ? id : counterId;
...
function patchSatirSil(id,sira){
	...
	silInp.onclick = function(){
		deleteRow("tablo_"+id + "_" +sira);
		rowDeleted(id);
	    return true;
	};
...
function rowDeleted(id){
	...
	//var satirInps = document.getElementsByName("input"+id+"-1[]");
	var satirInps =  getElementsByName_iefix("input", "input"+id+"-1[]")
