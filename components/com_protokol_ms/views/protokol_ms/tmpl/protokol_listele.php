...
<div class="form_item">
	<div class="form_element cf_heading">
		<h1 class="contentheading">Meslek Standardı Protokol Listesi</h1>
	</div>
...
<div class="form_content">
	<form id="protokolForm" name="protokolForm" method="post"
		action="index.php?option=com_protokol_ms&amp;task=etkisizlestir">
		...
		<div>
			<table id="protokolListe">
				...
				for ($i = 0; $i < count($protokoller); $i++)
				{
					...
					echo '<td><a href="index.php?option=com_protokol_ms&layout=yeni&protokolID='.$satir["PROTOKOL_ID"].'">'.$satir["ADI"].'</a></td>';
...
function OnButton1()
{
    document.protokolForm.action = "index.php?option=com_protokol_ms&amp;task=etkinlestir";
    document.protokolForm.submit();             // Submit the page
    return true;
}
function OnButton2()
{
    document.protokolForm.action = "index.php?option=com_protokol_ms&amp;task=etkisizlestir";
    document.protokolForm.submit();             // Submit the page
    return true;
}
function OnButton3()
{
	...
	{		
	    document.protokolForm.action = "index.php?option=com_protokol_ms&amp;task=sil";
	    document.protokolForm.submit();             // Submit the page
	}
    return true;
}
function OnButton4()
{
	...
	document.protokolForm.action = "index.php?option=com_protokol_ms&amp;layout=yeni&amp;protokolID="+editIndex;
    document.protokolForm.submit();             // Submit the page
    return true;
}
function OnButton5()
{
    document.protokolForm.action = "index.php?option=com_protokol_ms&amp;layout=yeni";
    document.protokolForm.submit();             // Submit the page
    return true;
}
...
function doThis()
{
	...
	if(count==1)
	{
		...
		var editIndex = jQuery('input[type=checkbox]').filter(':checked').val();
		jQuery("#protokolForm").attr("action", "index.php?option=com_protokol_ms&amp;layout=yeni&amp;protokolID="+editIndex);
