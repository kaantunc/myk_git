...
class Protokol_MsModelProtokol_Kaydet extends JModel {

	function  protokolleriEtkisizlestir($protokollerArr)
	{
	...
	function  protokolleriEtkinlestir($protokollerArr)
	{
		$db  = &JFactory::getOracleDBO();
		$result = '';
		for($i=0; $i<count($protokollerArr); $i++)
	...
	function protokolleriSil($protokollerArr, $messageType)
	{
		$message = "";
		for($i=0; $i<count($protokollerArr); $i++)
		{
			...
			{
				//DELETE FROM M_PROTOKOL_KURULUS
	...
	function protokolBilgileriDuzenle($protokolID, $protokolAdi, $imzaTarihi, $sayisi,$suresi, $filePath)
	{
		$db  = &JFactory::getOracleDBO();
	...
	function protokolEkle($protokolID, $protokolAdi, $imzaTarihi, $sayisi, $suresi, $filePath)
	{
		...
		$params = array ($protokolID, $protokolAdi, $imzaTarihi, $sayisi, 1, PM_PROTOKOLTURU_MESLEKSTANDARDI, FormParametrik::harfleriTexttenCikar($suresi), $filePath);
		$messageToReturn = $db->prep_exec_insert($sql, $params);			
	
		return $messageToReturn;
	}
	
	function protokolKurulusuEkle($protokolID, $kurulusID, $kurulusTuru)
	{
		$db  = &JFactory::getOracleDBO();
