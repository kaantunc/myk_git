...
class Protokol_MsController extends JController {
	...
	function protokolKaydet()
	{
		...
		if(strlen($protokolID)==0) // YENI EKLE
		{
			...
			$message = $this->protokoleKurulusEkle($protokolID, $seciliKurulusIDleri, $seciliKurulusIDleri);
			$message .= $this->protokolleSektorleriIliskilendir($protokolID, $sektorler);
			if($message)
				$message=JText::_("SUCCESS_MESSAGE");
			
			$this->setRedirect('index.php?option=com_protokol_ms&layout=yeni&protokolID='.$protokolID, $message);
		}
		else // DUZENLE
		{
			...
			$msSayisi = JRequest::getVar('msSayisiTextbox');
			...
			$message = $this->protokolDuzenle($protokolID, $protokolAdi, $imzaTarihi, $msSayisi,$suresi, $file_path);
	...
	function yeniProtokolEkle()
	{
		...
		$protokolAdi = JRequest::getVar('protokolAdiTextbox');
		$imzaTarihi  = JRequest::getVar('imzaTarihiDatePicker');
		$sayisi = JRequest::getVar('msSayisiTextbox');
		...
		if($result == false){ //ERROR
			$mainframe->redirect('index.php?option=com_protokol_ms&layout=yeni', JText::_("PROTOKOL_EKLE_ERROR_MESSAGE"), 'error');
		}

		return $protokolID;

	}
	function protokoleKurulusEkle ($protokolID, $seciliKurulusIDleri, $seciliKurulusRolleri)
	{
		$model 		 = $this->getModel('protokol_kaydet');
		$success = TRUE;
		if($protokolID !=-1)
	...
	function protokolDuzenle($protokolID, $protokolAdi, $imzaTarihi, $msSayisi, $suresi, $file_path)
	{
		$model 		 = $this->getModel('protokol_kaydet');
		$message	 = $model->protokolBilgileriDuzenle($protokolID, $protokolAdi, $imzaTarihi, $msSayisi, $suresi, $file_path);

		return $message;
	}
	
	function protokolKuruluslariniDuzenle($protokolID, $kuruluslar, $kurulusTurleri)
	{
		$model 		 = $this->getModel('protokol_kaydet');
	...
	function etkisizlestir()
	{
		...
		$this->setRedirect('index.php?option=com_protokol_ms', $message);
	}
	...
	function sil()
	{
		...
		$this->setRedirect('index.php?option=com_protokol_ms', $message, $messageType);
	}

	function indir (){
		$protokolID = JRequest::getVar('protokolID');
		$model = $this->getModel('protokol_ms');
