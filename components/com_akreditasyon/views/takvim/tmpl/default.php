...
function createTables(){
	...
	//yeterlilikleriAl();
		createTable("sinavTakvimi",
				headers
	);
	patchSatirEkleWithDatePick ("sinavTakvimi", headers,new Array ('2'), "sinavTakvimi");
	...
	if(<?php echo isset($_POST['mode'])? "1" : "0";?>){
		...
		xmlhttpPost(null, 'index.php?option=com_akreditasyon&task=takvimAl&format=raw',
				function (){
...
function getTakvimUpdate(str){
	...
	str= trim(str);
	if(str == "no"){
		alert("Daha önce seçilen yıl için denetim takvimi bildirmemişsiniz. Şimdi Bildirebilirsiniz.");
		...
		addNRow('sinavTakvimi','3','sinavTakvimi');
...
function merkezSecildi(merkezInpId, merkezInpVal){
	...
	xmlhttpPost(null, 'index.php?option=com_akreditasyon&task=merkezYeterlilik&format=raw',
			function (){
...
function yetUpdatePageFunction(str){
	...
	var id = 'inputsinavTakvimi-3-' + str2[0];
...
<div class="sinavGirisBaslik">Yıllık Denetim Takvimi Gir</div>

<form id="sinavTakvimiForm" action="?option=com_akreditasyon&task=takvimKaydet" method="post" onsubmit="return validate('sinavTakvimiForm')">
