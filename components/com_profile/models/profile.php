	...
	function getKurulusSinavGirenAndBasariliYeni($kId){
		...
		foreach($dataSinavaGiren as $key=>$row){
			...
			$sqlBelgeli = "SELECT COUNT(DISTINCT MBH.TC_KIMLIK) AS ADAY
				...
				INNER JOIN M_BELGELENDIRME_SINAV MBS ON(MBH.SINAV_ID = MBS.SINAV_ID)
				WHERE MBH.KURULUS_ID = ? AND MBS.SONUC_DURUMU = 2 AND MBH.YETERLILIK_ID = ?
				";
			...
			$sql = "SELECT
					...
					AND MBA.SINAV_TARIHI <= MBH.BELGE_BAS_TARIH AND MBA.BASARI_DURUMU != 1";
...
GROUP BY TO_CHAR(BELGE_BAS_TARIH, 'YYYY')";
	...
	public function ProkoluOlanKuruluslar(){
		$db = JFactory::getOracleDBO ();
		
		$sql = "SELECT * FROM AB_KURULUS_PROTOKOL";
		$data = $db->prep_exec($sql, array());
		
		$pro = array();
		foreach ($data as $row){
			$pro[$row['KURULUS_ID']] = $row;
		}
		return $pro;
	}
