...
<div class="anaDiv" id="ProDok">
	<table style="width: 100%; text-align:center" border="1">
		...
		<?php foreach($pro as $row){
			...
			echo '<td><a href="index.php?dl=kurulus/'.$kurulus['USER_ID'].'/ab_protokol/'.$row['PRO_DOK'].'" class="btn btn-sm btn-warning">Protokol Dökümanı</a></td>';
            echo '<td>'.$row['VERGI_KIMLIK_NO'].'</td>';
			echo '<td>%'.$row['KDV'].'</td>';
			if($this->canEdit){
				if($row['DEZAVANTAJ']){
					echo '<td><button type="button" class="btn btn-sm btn-danger" onclick="FuncDezUygulamaGuncelle('.$row['ID'].',0)">Dezavantaj Uygulamasını Kaldır</button></td>';
				}else{
					echo '<td><button type="button" class="btn btn-sm btn-success" onclick="FuncDezUygulamaGuncelle('.$row['ID'].',1)">Dezavantaj Uygulamasını Uygula</button></td>';
				}
			}else{
				if($row['DEZAVANTAJ']){
			...
			if($this->canEdit){
				echo '<td><button type="button" class="btn btn-sm btn-warning" onclick="FuncProDuzenle('.$row['ID'].')">Düzenle</button></td>';
			}
...
<div class="anaDiv">
	<button type="button" class="btn btn-sm btn-primary" id="ProEkle">Protokol Sözleşmesi Ekle</button>
</div>	
<?php } ?>
<div class="anaDiv">
<hr>
...
<div class="anaDiv" id="ProDonem">
	<table style="width: 100%; text-align:center" border="1">
		...
		<?php foreach($donem as $row){
			...
			if(!$this->canEdit){
				echo '<td><a target="_blank" class="btn btn-sm btn-primary" href="index.php?option=com_belgelendirme_abhibe&view=belgelendirme_abhibe">Hibe Kullanımı</a></td>';
			}
			if($this->canEdit){
				echo '<td><button type="button" class="btn btn-sm btn-warning" onclick="FuncDonemDuzenle('.$row['ID'].')">Düzenle</button></td>';
			}
			echo '</tr>';
		}?>
		</tbody>
...
<div class="anaDiv">
	<button type="button" class="btn btn-sm btn-primary" id="DonemEkle">Hibe Dönemi Ekle</button>
</div>
<?php } ?>
<div class="anaDiv">
<hr>
...
<form id="ProForm" method="post" enctype="multipart/form-data" action="index.php?option=com_profile&task=ABProKaydet">		
    <div class="anaDiv font20 fontBold hColor text-center">
    	AB Protokolü
    </div>
    <div class="anaDiv">
    	<div class="div30 font16 hColor">Protokol Tarihi:</div>
    ...
    <div class="anaDiv">
    	<div class="div30 font16 hColor">Protokol Dökümanı:</div>
    	<div class="div70">
    ...
    <div class="anaDiv">
    	<div class="div30 font16 hColor">KDV Tutarı: %</div>
    	<div class="div70"><input type="number" class="input-sm" name="proKDV"/></div>
    </div>
    <div class="anaDiv">
    	<div class="div50">
    		<button type="button" class="btn btn-sm btn-danger" id="ProIptal">İptal</button>
    	</div>
    	<div class="div50 text-right">
    		<button type="button" class="btn btn-sm btn-success" id="ProKaydet">Kaydet</button>
    	</div>
    </div>
...
<form id="DonemForm" method="post" enctype="multipart/form-data" action="index.php?option=com_profile&task=ABDonemKaydet">		
    ...
    <div class="anaDiv">
    	<div class="div50">
    		<button type="button" class="btn btn-sm btn-danger" id="DonemIptal">İptal</button>
    	</div>
    	<div class="div50 text-right">
    		<button type="button" class="btn btn-sm btn-success" id="DonemKaydet">Kaydet</button>
    	</div>
    </div>
    <input type="hidden" name="kId" value="<?php echo $kurulus['USER_ID'];?>"/>
    <input type="hidden" name="dId" value="0"/>
