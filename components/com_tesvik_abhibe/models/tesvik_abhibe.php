...
class Tesvik_AbhibeModelTesvik_Abhibe extends JModel {
	
	function TesvikAdaylarWithTarih($basTarih,$bitTarih){
		$db = JFactory::getOracleDBO ();
		
		$sql = "SELECT MBS.*, MY.*, MBO.*, MTI.DURUM, MTI.ITIRAZ_UCRET 
				...
				WHERE MBS.ABHIBE = 2 
				AND MBS.BELGENO NOT IN (SELECT BELGE_NO FROM AB_HIBE_ADAY WHERE ODENDI = 0 OR ODENDI = 1)
...
// 				'BelgeMasraf'=>$BelgeMasraf);
	...
	function TesvikAdayYarat($data){
		...
		$istekId = $db->getNextVal('SEQ_AB_HIBE_ISTEK');
		
		$sql = "INSERT INTO AB_HIBE_ISTEK (ID,USER_ID,TARIH,BIT_TARIH) VALUES(?,?,SYSDATE,TO_DATE(?))";
		if($db->prep_exec_insert($sql, array($istekId,$user_id,$data['bit_tarih']))){
			$sqlBelge = "INSERT INTO AB_HIBE_ADAY (TESVIK_ID,BELGE_NO) VALUES(?,?)";
			$hata = 0;
			...
			if($hata>0){
				$sqlDelete = "DELETE FROM AB_HIBE_ADAY WHERE TESVIK_ID = ?";
...
// 		$sql = "SELECT MA.TESVIK_ID, COUNT(MA.TESVIK_ID) AS ADAY, MT.ID, MT.TARIH, MT.USER_ID, MT.DURUM, TO_CHAR(MT.BIT_TARIH,'dd/mm/yyyy') AS ARAMA_TARIH FROM AB_HIBE_ADAY MA
// 				INNER JOIN AB_HIBE_ISTEK MT ON(MA.TESVIK_ID = MT.ID)
// 				GROUP BY MA.TESVIK_ID, MT.ID, MT.TARIH, MT.DURUM, MT.BIT_TARIH, MT.USER_ID
// 				ORDER BY TARIH DESC";
		$sql = "SELECT ID, TARIH, USER_ID, DURUM, TO_CHAR(BIT_TARIH,'dd/mm/yyyy') AS ARAMA_TARIH FROM AB_HIBE_ISTEK
				ORDER BY TARIH DESC";
	...
	function TesvikIstekleriUser($tesvik){
		...
		$sql = "SELECT COUNT(*) AS SAY FROM AB_HIBE_ADAY 
				WHERE TESVIK_ID = ?";
	...
	function GetTesvikWithTesvikId($tesvikId){
		$db = JFactory::getOracleDBO ();
		$sql = "SELECT ID, DURUM, TARIH, TO_CHAR(BIT_TARIH,'dd/mm/yyyy') AS BIT_TARIH, USER_ID,LISTE_KODU FROM AB_HIBE_ISTEK WHERE ID = ?";
	...
	function GetTesvikAdaylarWithTesvikId($tesvikId){
		$db = JFactory::getOracleDBO ();
		$sql = "SELECT BELGE_NO FROM AB_HIBE_ADAY WHERE TESVIK_ID = ?";
	...
	function TesvikAdaylarEditWithTarih($tesvikId,$bitTarih){
		...
		$sql = "SELECT MBS.*, MY.*, MBO.*, MTI.DURUM, MTI.ITIRAZ_UCRET FROM M_BELGE_SORGU MBS
				INNER JOIN M_YETERLILIK MY ON(MBS.YETERLILIK_ID = MY.YETERLILIK_ID)
				INNER JOIN M_BELGELENDIRME_OGRENCI MBO ON(MBS.TCKIMLIKNO = MBO.TC_KIMLIK)
				INNER JOIN AB_HIBE_ADAY MTA ON(MBS.BELGENO = MTA.BELGE_NO)
				LEFT JOIN AB_HIBE_ITIRAZ MTI ON(MBS.BELGENO = MTI.BELGENO)
	...
	function TesvikAdaylarWithBelgeNo($BelgeNo){
		...
		$birimUcretiHesabi = array();
		foreach ($tesvikAday as $row){
			$birimUcretiHesabi[$row['BELGENO']] = FormABHibeUcretHesabi::BasariliBirimUcretiHesabi($row['TC_KIMLIK'],$row['YETERLILIK_ID'], $row['SINAV_TARIHI'],$row['KURULUS_ID']);
			/* Tesvik tarihinden sonraki ilk sınav tarihi */
// 			$ilkSinav = FormUcretHesabi::TesviktenSonrakiIlkSinavTarihi($row['TCKIMLIKNO'],$row['YETERLILIK_ID']);
// 			$YetUcretiHesabi[$row['BELGENO']] = $this->YeterlilikUcretHesabi($row['YETERLILIK_ID'], $ilkSinav);
// 			$BelgeMasraf[$row['BELGENO']] = FormUcretHesabi::BelgeMasrafi($row['BELGE_DUZENLEME_TARIHI']);
...
// 		);
	...
	function TesvikAdayEditKaydet($data){
		...
		if(!empty($tesvikId) && is_numeric($tesvikId)){
			...
			$sqlDel = "DELETE FROM AB_HIBE_ADAY WHERE TESVIK_ID = ?";
	...
	function TesvikSil($tId){
		...
		if(!empty($tId) && is_numeric($tId)){
			...
			if($db->prep_exec_insert($sqlDel, array($tId))){
				$sqlDelIs = "DELETE FROM AB_HIBE_ISTEK WHERE ID = ?";
	...
	function TesvikOnayaSun($tId){
		...
		if(!empty($tId) && is_numeric($tId)){
			$sqlUp = "UPDATE AB_HIBE_ISTEK SET DURUM = 1 WHERE ID = ?";
		...
		if(!empty($tId) && is_numeric($tId) && !empty($uId) && is_numeric($uId)){
			...
			if($dat){
				...
				if($komiteArray == $OnayArray){
					$tesviklistekodu = "SELECT LISTE_KODU FROM AB_HIBE_ISTEK WHERE ROWNUM <2 ORDER BY LISTE_KODU DESC";
					$lkodu = $db->prep_exec($tesviklistekodu, array());
					$liste_kodu =str_pad(($lkodu[0]['LISTE_KODU']+1),3, "0", STR_PAD_LEFT);
					$sqlTesvikUp = "UPDATE AB_HIBE_ISTEK SET DURUM = 2,LISTE_KODU=? WHERE ID = ?";
					...
					if($dat){
						...
						if($pat){
							...
							$sqlTesvikUp = "UPDATE AB_HIBE_ISTEK SET DURUM = 1,LISTE_KODU='' WHERE ID = ?";
	...
	function TesvikGeriGonder($tId,$uId){
		...
		if(!empty($tId) && is_numeric($tId) && !empty($uId) && is_numeric($uId)){
			...
			if($dat){
				$sqlTesvikUp = "UPDATE AB_HIBE_ISTEK SET DURUM = 0,LISTE_KODU='' WHERE ID = ?";
				$dat = $db->prep_exec_insert($sqlTesvikUp, array($tId), true);
				if($dat){
					return true;
				}else{
...
// 		return array('AdayBilgi'=>$tesvikAday, 'UcretBilgi'=>$birimUcretiHesabi, 'YetUcret'=>$YetUcretiHesabi, 'BelgeMasraf'=>$BelgeMasraf);
	...
	function TesvikAdayUcretYazdir($tId){
		...
		$sql = "UPDATE AB_HIBE_ADAY SET SINAV_UCRET = ?, BIRIMLER = ?, KDVSIZ_UCRET = ?, 
				KDVLI_UCRET = ?, BELGE_MASRAF_UCRET = ? 
		...
		foreach($AdayBilgi as $row){
			...
			$sqlItiraz = "SELECT * FROM AB_HIBE_ITIRAZ WHERE BELGENO=? AND DURUM = 1";
			$datItiraz = $db->prep_exec($sqlItiraz, array($row['BELGENO']));
			
			if($datItiraz){
				$SinavUcret = str_replace(',', '.', $datItiraz[0]['ITIRAZ_UCRET']);
			}
			
			if($SinavUcret > str_replace(',', '.', $YetUcret[$row['BELGENO']]['UCRET'])){
				$kdvli = str_replace(',', '.', $YetUcret[$row['BELGENO']]['UCRET']);
			}else{
				$kdvli = $SinavUcret;
			}
			
			$kdvsiz = $kdvli-($kdvli*(0.00759));
			
			$parSinav = floor($SinavUcret*100)/100;
			$parDamgasiz = floor($kdvsiz*100)/100;
			$parDamgali = floor($kdvli*100)/100;
		...
		if($hata>0){
			$sqlUp = "UPDATE AB_HIBE_ADAY SET SINAV_UCRET = NULL, BIRIMLER = NULL, KDVSIZ_UCRET = NULL, KDVLI_UCRET = NULL WHERE TESVIK_ID = ?";
	...
	function generateTxt($tesvikid,$temp = false){
		...
		$sql = "SELECT  AB_HIBE_ADAY.BELGE_NO,
						AB_HIBE_ADAY.DAMGALI_UCRET AS UCRET,
						AB_HIBE_ADAY.BELGE_MASRAF_UCRET,
						AB_HIBE_ISTEK.ID AS TESVIK_ID,
						AB_HIBE_ISTEK.BANKA_ODEME_TARIHI AS BANKA_ODEME_TARIHI,
						AB_HIBE_ISTEK.TARIH AS TESVIK_TARIHI,
				   ...
				   FROM AB_HIBE_ADAY
			 INNER JOIN AB_HIBE_ISTEK ON AB_HIBE_ISTEK.ID = AB_HIBE_ADAY.TESVIK_ID
		LEFT OUTER JOIN M_BELGE_SORGU ON M_BELGE_SORGU.BELGENO = AB_HIBE_ADAY.BELGE_NO
		...
		$content  = "";
		$content .= str_pad('H',1);
		$content .= str_pad(ZIRAAT_MYK_KURUM_KODU,5,"0", STR_PAD_LEFT);				//$kurum_kodu
		$content .= str_pad(ZIRAAT_MYK_KURUM_ALT_KODU,3,"0", STR_PAD_LEFT);			//$kurum_alt_kodu
		$content .= str_pad($headerDatas['TESVIK_ID'],10,"0", STR_PAD_LEFT); 		//$bordro_no
		$content .= str_pad(date('Ymd',strtotime($headerDatas['TESVIK_TARIHI'])),8);			//$bordro_tarihi
		...
		foreach($datas as $data){
			...
			$content .= str_pad($detay,1);
			$content .= str_pad($toplam_adet,7, "0", STR_PAD_LEFT);       		//$toplam_adet
			if($data['BANKA_ODEME_TARIHI'] <> ""){
				$content .= str_pad(date('Ymd',strtotime(str_replace('/', '-', $data['BANKA_ODEME_TARIHI']))),8); 
			}else{
				$content .= str_pad("",8);
			}
			...
			$content .= str_pad($data['IBAN'],26);
			$content .= str_pad('',16);											//$alacak_kredikart_no
			$content .= str_pad($data['UCRET'],16, "0", STR_PAD_LEFT);			//tutar

			$content .= FormFactory2::mb_str_pad($data['ADI'],30,' ',STR_PAD_RIGHT);//str_pad($data['ADI'],30);
			$content .= FormFactory2::mb_str_pad($data['SOYADI'],30,' ',STR_PAD_RIGHT);//str_pad($data['SOYADI'],30);
			$content .= FormFactory2::mb_str_pad($data['BABA_ADI'],30,' ',STR_PAD_RIGHT);
			$content .= str_pad($data['TC_KIMLIK'],11);
			$content .= FormFactory2::mb_str_pad(mb_substr("MYK MeslekiYeterlilikBelgesi sınav ücreti iadesi hesabınıza yatırılmıştır.Mesleki Yeterlilik Kurumu",0,100,"UTF-8"),100,' ',STR_PAD_RIGHT);//$aciklama
			$content .= str_pad(substr(ltrim($data['TELEFON'],0),0,10),10);
			$content .= str_pad($data['EMAIL'],100)."\n";
		...
		$content .= str_pad($trailer,1);
		$content .= str_pad($toplam_adet,7, "0", STR_PAD_LEFT);
		$content .= str_pad(number_format($toplam_tutar,2,'.', ''),16, "0", STR_PAD_LEFT);
	...
	function explodeAndCommitTxt($path){
		...
		foreach ($odeme_array as $data){
			$tesvikid = $data['TESVIKID'];
			$sql = "SELECT AB_HIBE_ADAY.ID,AB_HIBE_ADAY.BELGE_NO 
					  FROM AB_HIBE_ADAY 
                INNER JOIN M_BELGE_SORGU ON M_BELGE_SORGU.BELGENO = AB_HIBE_ADAY.BELGE_NO 
                     WHERE AB_HIBE_ADAY.TESVIK_ID = ? AND 
			...
			if($data['STATUS'] == "E"){
				...
				$odeme_durum = "-1";
			}else if($data['STATUS'] == "İ"){
				$odeme_durum = "-2";
			}
			$sql_up = "UPDATE AB_HIBE_ADAY SET ODENDI = ?,ACIKLAMA = ? WHERE TESVIK_ID = ? AND BELGE_NO = ?";
			
			$db->prep_exec_insert($sql_up, array($odeme_durum,$data['STATUS_MESSAGE'],$data['TESVIKID'],$tesvik_aday[0]['BELGE_NO']));
		}
		
		$sql_up_txt = "UPDATE AB_HIBE_ISTEK SET AKIBET_TXT = ? WHERE ID = ?";
	...
	function afterTransferBankTxt($tesvikid){
		...
		$sql = "SELECT AKIBET_TXT FROM AB_HIBE_ISTEK WHERE ID = ?";
		$data = $db->prep_exec($sql, array($tesvikid));
		...
		$data = array('tesvikid'     => $tesvikid,
					  ...
					  'odenen_toplam_tutar' => $odenen_toplam_tutar,
	...
	function readTxtForPdf($tesvikid,$path = false){
		...
		$sql       = "SELECT BANKA_TXT FROM AB_HIBE_ISTEK WHERE ID = ?";
	...
	function TesvikItirazlar($durum = 0){
		$db = JFactory::getOracleDBO ();
		
		$sql = "SELECT * FROM AB_HIBE_ITIRAZ MTI 
	...
	function Dezavantajlilar($durum = 0){
		$db = JFactory::getOracleDBO ();
	
		$sql = "SELECT ABDEZ.*, MBO.*, MBS.* FROM AB_HIBE_DEZAVANTAJ_ADAY ABDEZ
				INNER JOIN M_BELGELENDIRME_HAK_KAZANANLAR MBHAK ON (ABDEZ.BELGE_NO = MBHAK.BELGE_NO)
				INNER JOIN M_BELGELENDIRME_OGRENCI MBO ON(MBHAK.TC_KIMLIK = MBO.TC_KIMLIK)
				INNER JOIN M_BELGELENDIRME_SINAV MBS ON(ABDEZ.SINAV_ID = MBS.SINAV_ID)
				WHERE ABDEZ.DURUM = ?";
		...
		$BelgeUcretBilgi = array();
		foreach($itirazlar as $row){
			$BelgeUcretBilgi[$row['BELGE_NO']] = $this->TesvikAdaylarWithBelgeNo($row['BELGE_NO']);
			$kurulus[$row['BELGE_NO']] = $this->getKurulusBilgi($row['KURULUS_ID']);
		}
	...
	function AjaxItirazWithBelgeNo($belgeNo){
		$db = JFactory::getOracleDBO ();
		$sql = "SELECT * FROM AB_HIBE_ITIRAZ 
				WHERE BELGENO = ?";
	...
	function AjaxItirazUcretKaydetWithBelgeNo($belgeNo,$ucret){
		$db = JFactory::getOracleDBO ();
		$sql = "UPDATE AB_HIBE_ITIRAZ SET ITIRAZ_UCRET = ?
				WHERE BELGENO = ?";
	...
	function AjaxItirazDurumGuncelleWithBelgeNo($belgeNo, $durum){
		$db = JFactory::getOracleDBO ();
		$sql = "UPDATE AB_HIBE_ITIRAZ SET DURUM = ?
				WHERE BELGENO = ?";
	...
	function TesvikBankayaGonder($tId, $dId){
		...
		$sql = "SELECT BANKA_ODEME_TARIHI FROM AB_HIBE_ISTEK WHERE ID = ?";
	...
	function TesvikOdenenVeSonuc($data){
		...
		$TesvikOdemesi = array();
		$sql = "SELECT COUNT(*) as SAY FROM AB_HIBE_ADAY WHERE TESVIK_ID = ? ";
		foreach($data as $row){
			if($row['DURUM'] == 6){
	...
	function TesvikOdenemeyen($tId){
		...
		$sql = "SELECT * FROM AB_HIBE_ADAY MTA
	...
	function istekDetay($istekId){
		...
		$sql = "SELECT ID,USER_ID,TARIH,BIT_TARIH,BANKA_ODEME_TARIHI FROM AB_HIBE_ISTEK WHERE ID=?";
	...
	function saveIstekDetay($istekId,$bankaodemetarihi){
		$db = JFactory::getOracleDBO();
		$sql= "UPDATE AB_HIBE_ISTEK SET BANKA_ODEME_TARIHI = ? WHERE ID = ?";
	...
	function DezAvantajDurumGuncelle($post){
		$db = JFactory::getOracleDBO();
		$sql = "UPDATE AB_HIBE_DEZAVANTAJ_ADAY SET DURUM = ? WHERE BELGE_NO = ?";
		return $db->prep_exec_insert($sql, array($post['dId'],urldecode($post['bNo'])));
	}
}