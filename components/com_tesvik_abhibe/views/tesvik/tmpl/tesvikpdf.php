...
$html .= '<td width="6%" align="center">'.$YetUcret[$row['BELGENO']]['KARAR_SAYI'].'</td>';
$html .= '<td width="6%" align="center">'.number_format(UcretDuzenle($YetUcret[$row['BELGENO']]['UCRET']),2,',','.').'</td>';

$uc = 0;
if(is_null($row['ITIRAZ_DURUMU']) || $row['ITIRAZ_DURUMU'] == -1 || $row['ITIRAZ_DURUMU'] === ''){
	foreach($UcretBilgi[$row['BELGENO']] as $cow){
		$uc += $cow['ucret'];
	}
}else{
	$uc += UcretDuzenle($row['ITIRAZ_UCRET']);
}
...
if(UcretDuzenle($YetUcret[$row['BELGENO']]['UCRET']) > $uc){
	$html .= '<td width="6%" align="center">'.number_format($uc,2,',','.').'</td>';
	$alinacak = $uc;
}else{
	$html .= '<td width="6%" align="center">'.number_format(UcretDuzenle($YetUcret[$row['BELGENO']]['UCRET']),2,',','.').'</td>';
	$alinacak = UcretDuzenle($YetUcret[$row['BELGENO']]['UCRET']);
}

$FonBürüt += $alinacak;
$FonNet += DamgasizHesapla($alinacak);

// $html .= '<td width="6%" align="center">'.DamgasizHesapla($alinacak).'</td>';
// $TemTarihArr = explode('/', $row['BELGE_DUZENLEME_TARIHI']);
// $TemTarih = $TemTarihArr[2].$TemTarihArr[1].$TemTarihArr[0];
// if($TemTarih <= '20150719'){
if($row['BELGE_MASRAF'] == 1){
	$html .= '<td width="6%" align="center">0,00</td>';
	$temmuzOncesi += UcretDuzenle($BelgeMasraf[$row['BELGENO']]);
	$temSay++;
}else{
	$html .= '<td width="6%" align="center">'.number_format(UcretDuzenle($BelgeMasraf[$row['BELGENO']]),2,',','.').'</td>';
	$TopBelgeMasraf += UcretDuzenle($BelgeMasraf[$row['BELGENO']]);
}
