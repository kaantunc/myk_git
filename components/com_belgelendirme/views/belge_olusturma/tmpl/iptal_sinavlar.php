...
<form id="SinavAraForm" method="POST" enctype="multipart/form-data" action="index.php?option=com_belgelendirme&view=belge_olusturma&layout=iptal_sinavlar">
    <div class="anaDiv">
...
<div class="anaDiv">
    ...
    foreach($sinavlar as $row){
        ...
        echo '<td align="center"><button type="button" class="btn btn-xs btn-danger" onclick="FuncSinavIptal('.$row['SINAV_ID'].')">Detay</button></td>';
        echo '<td>'.$row['IPTAL_EDEN'].'</td>';
        echo '</tr>';
    }
    ?>
	</tbody>
...
jQuery(document).ready(function(){
	...
	jQuery('#sinavAra').live('click',function(e){
		...
		jQuery.ajax({
			...
			data:'yeterlilik_id='+yet+'&kurulus_id='+kur+'&basTarih='+bas+'&bitTarih='+bit+'&durum=0',
...
function FuncSinavIptal(sId){
    ...
    jQuery('#SinavIptalFormu #KayitliIptalFile').html('');
    jQuery('#SinavIptalFormu #ipUser').html('');
    ...
    jQuery.ajax({
        ...
        if(dat){
            ...
            jQuery('#SinavIptalFormu textarea[name="IptalAcik"]').val(ipDat['IPTAL_ACIKLAMA']);
            jQuery('#SinavIptalFormu #ipUser').html(dat['IptalUser']);

            jQuery(ipFile).each(function(key,vall){
                var fileLink = "index.php?dl=SinavIptal/"+sId+"/"+vall['DOKUMAN'];
                var ekle = '<div class="anaDiv" id="ipFile_'+vall['ID']+'"><div class="div70"><a target="_blank" class="btn btn-xs btn-info" href="'+fileLink+'">'+vall['DOKUMAN']+'</a></div><div class="div30"><button type="button" class="btn btn-xs btn-danger" onclick="FuncIptalFileSil('+vall['ID']+')"><i class="fa fa-minus"></i> Sil</button></div></div>';
                jQuery('#SinavIptalFormu #KayitliIptalFile').append(ekle);
            });
...
function FuncIptalFileSil(fId){
    if(confirm('Kayıtlı İptal Dosyasını Silmek İstediğinizden Emin misiniz?')){
        jQuery.ajax({
            async: false,
            type:'POST',
            url:'index.php?option=com_belgelendirme&task=AjaxSinavIptalFileSil&format=raw',
            data:'fId='+fId
        }).done(function(data){
            var dat = jQuery.parseJSON(data);
            if(dat){
                jQuery('#KayitliIptalFile #ipFile_'+fId).remove();
                alert('İptal Dosyası Silindi.');
            }else{
                alert('Bir hata meydana geldi. Lütfen tekrar deneyin.');
            }
        });
...
<div id="SinavIptalDiv" style=" width: 50%; min-height:100px; background-color: white; border:1px solid #00A7DE; display: none; padding:20px">
    <form method="POST" id="SinavIptalFormu" action="index.php?option=com_belgelendirme&view=belge_olusturma&layout=yapilmayan_sinavlar&task=program_iptal" enctype="multipart/form-data">
        ...
        <div class="anaDiv" style="display: inline-flex">
            ...
            <div class="div70">
                <div class="anaDiv" id="KayitliIptalFile">

                </div>
                <div class="anaDiv" id="iptalFiles">
