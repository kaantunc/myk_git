...
class BelgelendirmeModelBelgelendirme_Islemleri extends JModel {
	...
	function DegerlendiriciSinavKaydet ($post){
		...
		if(count($yeterlilik_id)>0){
			...
			foreach($yeterlilik_id as $tow){
				$sql_control = "SELECT * FROM M_BELGELENDIRME_DGRLNDRC_KRLS WHERE TC_KIMLIK = ? AND YETERLILIK_ID = ? AND KURULUS_ID = ?";
				$data_control = $_db->prep_exec($sql_control, array($tcno,$tow,$user_id));
				
				if(count($data_control) > 0){
					$sql_tekrar_gonder = "UPDATE M_BELGELENDIRME_DGRLNDRC_KRLS SET ONAY_BEKLEYEN_DGRLNDRC = ?, OLCUT_KARSILAMA_ACIKLAMA = ? WHERE TC_KIMLIK = ? AND YETERLILIK_ID = ? AND KURULUS_ID = ?";
					$_db->prep_exec_insert($sql_tekrar_gonder, array('0',$post['olcut_karsilama_aciklama_'.$tow],$tcno,$tow,$user_id));
     ...
     * @return mixed
     */
    function ProgramKaydet($post, $files){
		...
		$yeterlilik_id = $post['yets'];
		$date = $post['bastar'];
        $bassaat = $post['bassaat'];
        $date .= ' '.$bassaat;
		$ilId = $post['il'];
        $nextId = $_db->getNextVal('SEQ_SINAVPROGRAM');
		$sql = "INSERT INTO M_BELGELENDIRME_SINAV (SINAV_ID,KURULUS_ID,YETERLILIK_ID,BASLANGIC_TARIHI,SINAV_ILI,BILDIRIM_DURUMU, BILDIRIM_TARIHI)
				VALUES (?,?,?,TO_DATE(?, 'dd-mm-yyyy HH24:MI'),?,1,SYSDATE)";
	
		$params = array(
				$nextId,$user_id,$yeterlilik_id,$date,$ilId
		);
        $return['MESSAGE'] = '';
    	if($_db->prep_exec_insert($sql, $params)){
            $donen = $this->getAdayExcel(array(), array('sinav'=>$nextId), $files, false);
            if(array_key_exists('hataMesaji',$donen)){
                if(is_array($donen['hataMesaji'])){
                    $donenMesaj = '';
                    foreach($donen['hataMesaji'] as $key=>$mesaj){
                        switch ($key){
                            case 1:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['hatalıTckimlik'] as $tckimlik){
                                    $donenMesaj .= "<li>Satır No: ".$tckimlik[0].", TC Kimlik No:".$tckimlik[1]."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 2:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['tckimlik'] as $tckimlik){
                                    $donenMesaj .= "<li>Satır No: ".$tckimlik[0].", TC Kimlik No:<a href='#' id='tcDeg'>".$tckimlik[1]."</a></li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 3:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['sinavTuruKoduSatir'] as $satir){
                                    $donenMesaj .= "<li>Satır No: ".$satir."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 4:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['sinavTarihi'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0].", Sınav Tarihi: ".$satir[1]."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 5:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['sinavYeri'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0].", Sınav Yeri: ".$satir[1]."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 6:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['degerlendirici'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0].", Değerlendirici TCKN: ".$satir[1]."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 7:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['mukerrer'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 9:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['sinavTarihi360'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0].", Sınav Tarihi: ".$satir[1]."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 10:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['hataliCins'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0]."</li>";
                                }
                                foreach($donen['hataliEgitim'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0]."</li>";
                                }
                                foreach($donen['hataliCalisma'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0]."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 11:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['hataliIban'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0].", Iban: ".$satir[1]."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            case 12:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['hataliEmail'] as $satir){
                                    $donenMesaj .= "<li>Satır no: ".$satir[0].", Email: ".$satir[1]."</li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                            ...
                            case 14:
                                $donenMesaj .=  "<h2>".$mesaj."</h2>";
                                foreach($donen['tckimlik_sistem'] as $tckimlik){
                                    $donenMesaj .= "<li>Satır No: ".$tckimlik[0].", TC Kimlik No:<a href='#' id='tcDeg'>".$tckimlik[1]."</a></li>";
                                }
                                $donenMesaj .= "<hr>";
                                break;
                    ...
                    $return['MESSAGE'] = $donenMesaj;
                }else{
                    $return['MESSAGE'] = $donen['hataMesaji'];
                }
                $return['STATUS'] = 0;
                $sqlDel = "DELETE FROM M_BELGELENDIRME_SINAV WHERE SINAV_ID = ?";
                $_db->prep_exec_insert($sqlDel,array($nextId));
            }else {
                $return['STATUS'] = 1;
		...
		$_db = JFactory::getOracleDBO ();
                if(count($post)>0){
                    $sql = "SELECT M_BELGELENDIRME_SINAV.*, M_YETERLILIK.*,
                              TO_CHAR(M_BELGELENDIRME_SINAV.BASLANGIC_TARIHI,'HH24:MI') AS BASSAAT
                    ...
                    $sql = "SELECT M_BELGELENDIRME_SINAV.*, M_YETERLILIK.*,TO_CHAR(M_BELGELENDIRME_SINAV.BASLANGIC_TARIHI,'HH24:MI') AS BASSAAT
                    		  FROM M_BELGELENDIRME_SINAV
							  JOIN M_YETERLILIK ON M_BELGELENDIRME_SINAV.YETERLILIK_ID = M_YETERLILIK.YETERLILIK_ID
	...
	function getGecmisProgram($user_id,$post){
		$_db = JFactory::getOracleDBO ();
                ...
                if(count($post)>0){
                    $sql = "SELECT M_BELGELENDIRME_SINAV.*, M_YETERLILIK.*,TO_CHAR(M_BELGELENDIRME_SINAV.BASLANGIC_TARIHI,'HH24:MI') AS BASSAAT  FROM M_BELGELENDIRME_SINAV
							  JOIN M_YETERLILIK ON M_BELGELENDIRME_SINAV.YETERLILIK_ID = M_YETERLILIK.YETERLILIK_ID
    ...
    function getYapilmayanProgram($user_id,$post){
        ...
        $sql = "SELECT M_BELGELENDIRME_SINAV.*, M_YETERLILIK.*,TO_CHAR(M_BELGELENDIRME_SINAV.BASLANGIC_TARIHI,'HH24:MI') AS BASSAAT FROM M_BELGELENDIRME_SINAV
                                JOIN M_YETERLILIK ON M_BELGELENDIRME_SINAV.YETERLILIK_ID = M_YETERLILIK.YETERLILIK_ID
                                WHERE M_BELGELENDIRME_SINAV.IPTAL_USER IS NULL AND M_BELGELENDIRME_SINAV.SONUC_DURUMU = 1 AND M_BELGELENDIRME_SINAV.GECERLILIK_DURUMU = 1
                                AND M_BELGELENDIRME_SINAV.BASLANGIC_TARIHI < (Select TRUNC(SYSDATE) from DUAL)
                                AND M_BELGELENDIRME_SINAV.SINAV_ID NOT IN (SELECT DISTINCT SINAV_ID FROM M_BELGELENDIRME_ADAY_BILDIRIM WHERE KURULUS_ID = ?)
                                AND M_BELGELENDIRME_SINAV.KURULUS_ID=? ";
        ...
        $sql .= " ORDER BY M_BELGELENDIRME_SINAV.BASLANGIC_TARIHI DESC";
		return $_db->prep_exec($sql, $param);
    }

    function getIptalProgram($user_id,$post){
        $_db = JFactory::getOracleDBO ();

        $sql = "SELECT M_BELGELENDIRME_SINAV.*, M_YETERLILIK.*,TO_CHAR(M_BELGELENDIRME_SINAV.BASLANGIC_TARIHI,'HH24:MI') AS BASSAAT FROM M_BELGELENDIRME_SINAV
							  JOIN M_YETERLILIK ON M_BELGELENDIRME_SINAV.YETERLILIK_ID = M_YETERLILIK.YETERLILIK_ID
                              WHERE M_BELGELENDIRME_SINAV.IPTAL_USER IS NOT NULL
                              AND M_BELGELENDIRME_SINAV.KURULUS_ID=? AND M_BELGELENDIRME_SINAV.GECERLILIK_DURUMU=2 ";
        $param = array($user_id);
        if(isset($post['yeter']) && $post['yeter'] != ''){
            $sql .= " AND M_BELGELENDIRME_SINAV.YETERLILIK_ID=?";
            $param[] = $post['yeter'];
        ...
        $data =  $_db->prep_exec($sql, $param);
        foreach($data as $key=>$val){
            $data[$key]['IPTAL_EDEN'] = '';
            $kurData = FormFactory::getKurulusBilgi($val['IPTAL_USER']);
            if($kurData){
    ...
    function ProgramSil($post,$files){
        $_db = JFactory::getOracleDBO ();
        $user 	 = &JFactory::getUser ();
        $user_id = $user->getOracleUserId ();

        $sId = $post['sId'];
        $aciklama = $post['IptalAcik'];
        $canEdit = array_key_exists('canEdit',$post)?$post['canEdit']:false;
        $IptalFile = array_key_exists('IptalFile',$files)?$files['IptalFile']:false;
        $FileHata = 0;

        $sqlPlus = "";
        if(!$canEdit){
            $sqlPlus = " AND KURULUS_ID = ".$user_id;
        }
        $returns = array();

        $sql="SELECT * FROM M_BELGELENDIRME_SINAV WHERE SINAV_ID=?".$sqlPlus;
        $data = $_db->prep_exec($sql, array($sId));
        if(count($data)>0){
            if($IptalFile){
                $directory = EK_FOLDER."SinavIptal/".$sId;
                if (!file_exists($directory)){
                    mkdir($directory, 0700,true);
                }
                for($i = 0; $i<count($IptalFile['name']); $i++){
                    if($IptalFile['type'][$i] == 'image/png' || $IptalFile['type'][$i] == 'image/jpeg' || $IptalFile['type'][$i] == 'application/zip'
                        || $IptalFile['type'][$i] == 'application/x-zip' || $IptalFile['type'][$i] == 'application/x-rar-compressed'
                        || $IptalFile['type'][$i] == 'application/pdf' || $IptalFile['type'][$i] == 'application/x-7z-compressed'){
                        $fileName = explode('.',$IptalFile['name'][$i]);
                        $name = $i.date('YmdHis').'.'.$fileName[count($fileName)-1];
                        $path = $directory.'/'.$name;
                        if(!move_uploaded_file($IptalFile['tmp_name'][$i], $path)){
                            $FileHata++;
                        }else{
                            $nextId = $_db->getNextVal('SEQ_SINAVIPTAL');
                            $sqlIns = "INSERT INTO M_BELGELENDIRME_SINAV_IPTAL (ID,SINAV_ID,DOKUMAN,USER_ID)
							VALUES(?,?,?,?)";
                            if(!$_db->prep_exec_insert($sqlIns,array($nextId,$sId,$name,$user_id))){
                                $FileHata++;
                            }
                        }
                    }
                ...
                if($FileHata > 0){
                    $return['hata'] = true;
                    $return['message'] = 'İptal Dosyası Yüklenirken Bir Hata Meydana Geldi. Dosyların Formatlarına Dikkat Ediniz.';
                    $sqlDel = "DELETE FROM M_BELGELENDIRME_SINAV_IPTAL WHERE SINAV_ID = ?";
                    $_db->prep_exec_insert($sqlDel,array($sId));
                    return $return;
                }
            }

            $hata = 0;
            $sql = "UPDATE M_BELGELENDIRME_SINAV SET GECERLILIK_DURUMU = 2, IPTAL_ACIKLAMA = ?, IPTAL_USER = ? WHERE SINAV_ID = ?";
            if(!$_db->prep_exec_insert($sql, array($aciklama,$user_id,$sId))){
                $hata++;
            }

            $sql = "DELETE FROM M_BELGELENDIRME_ADAY_BILDIRIM WHERE SINAV_ID = ?";
            if(!$_db->prep_exec_insert($sql, array($sId))){
                $hata++;
            }

            if($FileHata > 0 || $hata > 0){
                $return['hata'] = true;
                $return['message'] = 'İptal Dosyası Yüklenirken Bir Hata Meydana Geldi. Dosyların Formatlarına Dikkat Ediniz.';
                $sqlDel = "DELETE FROM M_BELGELENDIRME_SINAV_IPTAL WHERE SINAV_ID = ?";
                $_db->prep_exec_insert($sqlDel,array($sId));
                $sql = "UPDATE M_BELGELENDIRME_SINAV SET GECERLILIK_DURUMU = 1 WHERE SINAV_ID = ?";
                $_db->prep_exec_insert($sql, array($sId));
                return $return;
            }else{
                $return['hata'] = false;
            }
    ...
    function AjaxSinavIptalBilgi($post){
        $_db = JFactory::getOracleDBO ();
        $sId = $post['sId'];
        $sql = "SELECT * FROM M_BELGELENDIRME_SINAV WHERE SINAV_ID = ?";
        $data = $_db->prep_exec($sql,array($sId));

        $sql = "SELECT * FROM M_BELGELENDIRME_SINAV_IPTAL WHERE SINAV_ID = ?";
        $dataFile = $_db->prep_exec($sql,array($sId));

        if($data){
            $IptalUserId= $data[0]['IPTAL_USER'];
            $kurData = FormFactory::getKurulusBilgi($IptalUserId);
            if($kurData){
                $IptalUser = $kurData['KURULUS_ADI'];
            }else{
                $mysqlDB = &JFactory::getDBO();
                $sqlIptalUser = "SELECT name FROM #__users as users
					WHERE tgUserId = ".$IptalUserId;
                $mysqlDB->setQuery($sqlIptalUser);
                $ipUser = $mysqlDB->loadObjectList();
                $IptalUser = $ipUser[0]->name;
            }

            return array('data'=>$data[0], 'dataFile'=>$dataFile, 'IptalUser'=>$IptalUser);
        }else{
            return false;
        }
    }

    function AjaxSinavIptalFileSil($post){
        $_db = JFactory::getOracleDBO ();
        $fId = $post['fId'];
        $sql = "DELETE FROM M_BELGELENDIRME_SINAV_IPTAL WHERE ID = ?";
        return $_db->prep_exec_insert($sql,array($fId));
    }
			...
			foreach ($datas as $data){
				if(array_key_exists(($data['YETERLILIK_ID']."-".$data['SINAV_TURU']), $post_yeterlilik)){
					...
					if($uygunluk_formu['size'] > 0 && $uygunluk_formu['error']==0 && $uygunluk_formu['size']<30000000){
						...
						$directory = EK_FOLDER."sinavMerkeziUygunlukFormu/".$post['sinavyerid']."/".$yeterlilik_id."/".$sinav_turu;
						if (!file_exists($directory)){
							mkdir($directory, 0700,true);
						}
						$normalFile = FormFactory::formatFilename ($uygunluk_formu['name']);
						$path = "sinavMerkeziUygunlukFormu/".$post['sinavyerid']."/".$yeterlilik_id."/".$sinav_turu."/".$normalFile;
					...
					$_db->prep_exec($sql_delete, array($post['sinavyerid'],$data['YETERLILIK_ID'],$data['SINAV_TURU']));
				}
			}
			...
			if($files['upload']['type'] == 'application/vnd.ms-excel'){
				...
				for($i = 4 ; $i <= count($excel->sheets[0]['cells']) ; $i++){
					...
					foreach($excel->sheets[0]['cells'][$i] as $key => $val){
						...
						}else if($cols[$key]=="tckn" && $data[$x][$cols[$key]] == ''){
							$bossatir++;
							break;
						}else {
							if($data[$x][$cols[$key]] != '' ){
...
// 					if($kayitliaday[0]['ADI'] != trim(FormFactory::ucWordsTR($satir['adi'])) || $kayitliaday[0]['SOYADI'] != trim(FormFactory::toUpperCase($satir['soyadi'])) || $kayitliaday[0]['DOGUM_TARIHI'] != trim($satir['dogumtarihi']) || $kayitliaday[0]['DOGUM_YERI'] != trim(FormFactory::ucWordsTR($satir['dogumyeri'])) || $kayitliaday[0]['BABA_ADI'] != trim(FormFactory::ucWordsTR($satir['babaadi'])) || $kayitliaday[0]['CINSIYETI'] != $Cins || $kayitliaday[0]['EGITIMI'] != $egitimi || $kayitliaday[0]['CALISMA_DURUMU'] != $calismaDurum){
	...
	function tckimlik($tckimlik){
		...
		if($tckimlik[0]==0 or !ctype_digit($tckimlik) or strlen($tckimlik)!=11){ return false;  }
			...
			if($pest != $tckimlik[9] || $tumt%10 != $tckimlik[10]){ return false; }
			else{
...
//                  $objPHPExcel = PHPExcel_IOFactory::load($files['upload']['tmp_name']);
                    ...
                    for($i = 4 ; $i <= count($excel->sheets[0]['cells']) ; $i++){
                    	...
                    	foreach($excel->sheets[0]['cells'][$i] as $key => $val){
                    		...
                    		}else if($cols[$key]=="tckn" && $data[$x][$keySql[$key]] == ''){
                    			$bossatir++;
                    			break;
                    		}else {
...
//             $dataSinavIds = $_db->prep_exec_array($sinavIdsSql, array($sinav_id,$sinav_id));
            ...
            foreach($Adaylar as $aday){
            	...
            	if(count($sinavBirBirimBasariZor) > 0 || count($sinavBirBirimBasariSec) > 0){
	                $sonucBirim = $this->yeterlilikBelgeHakki($aday['TC_KIMLIK'],$yeterlilik_id,$alternatifTipi,$dataYet,$sinavTarihi, null, $user_id);
	                if($sonucBirim != false){
	                    $basariliBirimler[(string)$aday['TC_KIMLIK']] = $sonucBirim;   
	                }
//                 else{
        ...
        function BirimdenBasarilimiAday($tckn,$birim_id,$sinavTurleri,$sinavTarihi,$yeniMi,$user_id){
        	...
        	if($yeniMi == 1){
        		...
        		if($biriTur){
        			...
        			// Fakat birim başarı tarihi geçmemiş olabilir.
...
// 			$mailGorevli = array('ktunc@myk.gov.tr');
           ...
           foreach($belgeAday as $adayHak){
               ...
               // Adayın Daha önce Aynı belgeno ile belgesi var mı?
               $sql = "SELECT * FROM M_BELGE_SORGU MBS 
               		INNER JOIN M_BELGELENDIRME_HAK_KAZANANLAR MBH ON(MBS.BELGENO = MBH.BELGE_NO AND MBS.TCKIMLIKNO = MBH.TC_KIMLIK) 
               		WHERE MBH.ID = ?";
               $BelgeVarmi = $_db->prep_exec($sql, array($adayHak));
               // Adayın Daha önce Aynı belgeno ile belgesi var mı? SON
...
// 			$sqlTesvkiVarmi = "SELECT * FROM M_BELGE_SORGU WHERE TCKIMLIKNO = ? AND TESVIK != 0";
			$sqlTesvkiVarmi = "SELECT * FROM M_BELGELENDIRME_HAK_KAZANANLAR WHERE TC_KIMLIK = ? AND TESVIK = 1";
...
// 			$sqlTesvkiVarmi = "SELECT * FROM M_BELGE_SORGU WHERE TCKIMLIKNO = ? AND ABHIBE != 0 AND YETERLILIK_ID = ?";
			$sqlTesvkiVarmi = "SELECT * FROM M_BELGELENDIRME_HAK_KAZANANLAR WHERE TC_KIMLIK = ? AND TESVIK = 2";
			$tesData = $_db->prep_exec($sqlTesvkiVarmi, array($val));
			if($tesData){
				$tesvikArray[$val] = false;
			}else{
    ...
    function SonBelgeNo($yetId){
        ...
        $yetBelgeKod = trim(str_replace(' ','',str_replace('-','',$kurs['KURULUS_YETKILENDIRME_NUMARASI']).'/'.$yeterlilik[0]['YETERLILIK_KODU']));

        // Yeni Kod
        $sqlBS = "SELECT SONBELGENO FROM (SELECT TO_NUMBER(SUBSTR(REPLACE(MBS.BELGENO, '".$yetBelgeKod."/', ''),INSTR(REPLACE(MBS.BELGENO, '".$yetBelgeKod."/', ''),'/')+1)) as SONBELGENO FROM M_BELGE_SORGU MBS WHERE MBS.BELGENO LIKE '".$yetBelgeKod."%'
                    ORDER BY SONBELGENO DESC) WHERE ROWNUM = 1";
        $dataBS = $_db->prep_exec($sqlBS,array());

        $sqlBH = "SELECT SONBELGENO FROM (SELECT TO_NUMBER(SUBSTR(REPLACE(MBS.BELGE_NO, '".$yetBelgeKod."/', ''),INSTR(REPLACE(MBS.BELGE_NO, '".$yetBelgeKod."/', ''),'/')+1)) as SONBELGENO FROM M_BELGELENDIRME_HAK_KAZANANLAR MBS WHERE MBS.BELGE_NO LIKE '".$yetBelgeKod."%'
                    ORDER BY SONBELGENO DESC) WHERE ROWNUM = 1";
        $dataBH = $_db->prep_exec($sqlBH,array());

        $sonBH = 0;
        if($dataBH){
            $sonBH = $dataBH[0]['SONBELGENO'];
        }

        $sonBS = 0;
        if($dataBS){
            $sonBS = $dataBS[0]['SONBELGENO'];
        }
        ...
        $belgeNoSon = trim(str_replace(' ','',$yetBelgeKod.'/'.$yeterlilik[0]['REVIZYON'].'/'.$sonBelgeNo));
        return array($belgeNoSon,$sonBelgeNo,$yetBelgeKod.'/'.$yeterlilik[0]['REVIZYON']);
        // Yeni KOD Son

    }
        
	function SonBelgeNoEski($yetId){
		$_db = JFactory::getOracleDBO ();
		$user 	 = &JFactory::getUser ();
		$user_id = $user->getOracleUserId();
...
// 			$_db->prep_exec_insert($sql, array($yetBelgeKod,$sonBelgeNo,$user_id,$yetId));
	...
	function BelgeNoSonucGonder($post, $files){
		...
		foreach($belgeNo as $key=>$val){
			...
			if($sonucBirim != false){
				...
				$hak_id = $_db->getNextVal('SEQ_HAK_KAZANAN');
				$sql = "INSERT INTO M_BELGELENDIRME_HAK_KAZANANLAR "
						. "(ID,TC_KIMLIK,YETERLILIK_ID,SINAV_ID,KURULUS_ID,SINAV_TARIHI,AKTIF,BELGE_NO,BELGE_BAS_TARIH,ACIKLAMA,BASVURU_ID, TESVIK) "
						. "VALUES(?,?,?,?,?,?,?,?,TO_DATE(?, 'dd/mm/yyyy'),?)";
	...
	function SinavYapilanYeterlilikler($durum = -1){
        ...
        if($durum == 3){ // İptal Edilen
            ...
            $sql = "select distinct m_yeterlilik.*
					...
					where MBS.IPTAL_USER IS NULL AND ((MBS.bildirim_durumu = 0 AND MBS.BASLANGIC_TARIHI<TO_DATE(sysdate, 'dd/mm/yyyy'))
					OR (MBS.bildirim_durumu=1 AND MBS.BASLANGIC_TARIHI<=TO_DATE(sysdate, 'dd/mm/yyyy') AND MBS.SINAV_ID NOT IN (SELECT DISTINCT SINAV_ID FROM M_BELGELENDIRME_SINAV_DOSYA)))
					order by m_yeterlilik.yeterlilik_adi asc, m_yeterlilik.yeterlilik_kodu asc, m_yeterlilik.revizyon asc";
        }else if($durum == 4){ // Yapılacak Sınavlar
            $sql = "select distinct m_yeterlilik.*
					 from m_belgelendirme_sinav MBS
					  join m_yeterlilik on MBS.yeterlilik_id = m_yeterlilik.yeterlilik_id
            		where MBS.sonuc_durumu = 1 AND MBS.GECERLILIK_DURUMU = 1 AND MBS.bildirim_durumu = 1
					AND MBS.BASLANGIC_TARIHI >= TO_DATE(sysdate, 'dd/mm/yyyy')
					order by m_yeterlilik.yeterlilik_adi asc, m_yeterlilik.yeterlilik_kodu asc, m_yeterlilik.revizyon asc
					   ";
        }else if($durum == 2){ // Yapılan sınavlar
            $sql = "select distinct SG.yeterlilik_kodu, SG.seviye_id, SG.yeterlilik_adi,SG.REVIZYON
				...
				order by SG.yeterlilik_adi asc, SG.yeterlilik_kodu asc, SG.revizyon asc
				 ";
        }else if($durum == 1){ // Sonuc Bildirilicek Sınavlar
            $sql = "select distinct m_yeterlilik.*
					 from m_belgelendirme_sinav MBS
					  join m_yeterlilik on MBS.yeterlilik_id = m_yeterlilik.yeterlilik_id
            		where MBS.sonuc_durumu = 1 AND MBS.GECERLILIK_DURUMU = 1 AND MBS.bildirim_durumu = 1
            ...
            $sql = "select distinct yeterlilik_id, yeterlilik_adi, yeterlilik_kodu,revizyon
				from m_belgelendirme_sinav join m_yeterlilik using(yeterlilik_id)
				order by yeterlilik_adi asc, yeterlilik_kodu asc, revizyon asc";
        }
	...
	function SinavYapanKuruluslar($durum = -1){
		$_db = JFactory::getOracleDBO();

        if($durum == 3){ // İptal Edilen
            $sql = "select distinct kurulus_id
				from m_belgelendirme_sinav
				where iptal_user is not null and gecerlilik_durumu = 2";
        }else if ($durum == 0){ // Yapilamayan
            $sql = "select distinct MBS.kurulus_id
					 from m_belgelendirme_sinav MBS
					where MBS.IPTAL_USER IS NULL AND ((MBS.bildirim_durumu = 0 AND MBS.BASLANGIC_TARIHI<TO_DATE(sysdate, 'dd/mm/yyyy'))
					OR (MBS.bildirim_durumu=1 AND MBS.BASLANGIC_TARIHI<=TO_DATE(sysdate, 'dd/mm/yyyy') AND MBS.SINAV_ID NOT IN (SELECT DISTINCT SINAV_ID FROM M_BELGELENDIRME_SINAV_DOSYA)))";
        }else if($durum == 4){ // Yapılacak Sınavlar
            ...
            $sql = "select distinct SG.kurulus_id
				from (select yeterlilik_kodu, seviye_id, yeterlilik_adi,REVIZYON, yeterlilik_id,count(tc_kimlik) as sinava_girmis,
				            sinav_id, BASLANGIC_TARIHI,SINAV_ILI, KURULUS_ID
				                from (select distinct tc_kimlik, seviye_id, yeterlilik_kodu, yeterlilik_adi,yeterlilik_id,
				                                            sinav_id, BASLANGIC_TARIHI,SINAV_ILI, KURULUS_ID, REVIZYON from m_belgelendirme_sinav
        ...
        foreach($data as $row){
            ...
            $dataKurArr[$row['KURULUS_ID']] = $dataKur[$row['KURULUS_ID']]['KURULUS_ADI'];
        }

        array_multisort($dataKurArr, SORT_ASC, $dataKur);
		return $dataKur;
	}
...
// 				unlink($filename);
    ...
    public function AjaxGetSinavIli($post){
        $db  = JFactory::getOracleDBO ();
        $sql = "SELECT * FROM M_BELGELENDIRME_SINAV WHERE SINAV_ID = ?";
        $data = $db->prep_exec($sql,array($post['sId']));
        if($data){
            return array('hata'=>false,'sId'=>$data[0]['SINAV_ID'],'sIl'=>$data[0]['SINAV_ILI']);
        }else{
            return array('hata'=>true);
        }
    }

    public function AjaxSinavIliKaydet($post){
        $db  = JFactory::getOracleDBO ();
        $sql = "UPDATE M_BELGELENDIRME_SINAV SET SINAV_ILI = ? WHERE SINAV_ID = ?";
        $data = $db->prep_exec_insert($sql,array($post['sIl'],$post['sId']));
        return $data;
    }

    public function sinavTarihSaatGecmisMi($sId){
        $db  = JFactory::getOracleDBO ();
        $sql = "SELECT * FROM M_BELGELENDIRME_SINAV WHERE SINAV_ID = ? AND BASLANGIC_TARIHI > SYSDATE";
        $data = $db->prep_exec($sql,array($sId));
        if($data){
            return false;
        }else{
            return true;
    ...
    function GetSinavlariAra($post, $durum = 1){
        ...
        if($durum == 0){
            ...
            if(!empty($post['basTarih']) && !empty($post['bitTarih'])){
                $sql .= " and MBS.BASLANGIC_TARIHI >= TO_DATE('".$post['basTarih']."', 'dd/mm/yyyy') and  MBS.BASLANGIC_TARIHI <= TO_DATE('".$post['bitTarih']."', 'dd/mm/yyyy')";
                $sqlKur .= " and MBS.BASLANGIC_TARIHI >= TO_DATE('".$post['basTarih']."', 'dd/mm/yyyy') and  MBS.BASLANGIC_TARIHI <= TO_DATE('".$post['bitTarih']."', 'dd/mm/yyyy')";
            }
            else if(!empty($post['basTarih'])){
                $sql .= " and MBS.BASLANGIC_TARIHI >= TO_DATE('".$post['basTarih']."', 'dd/mm/yyyy')";
                $sqlKur .= " and MBS.BASLANGIC_TARIHI >= TO_DATE('".$post['basTarih']."', 'dd/mm/yyyy')";
            }
            else if(!empty($post['bitTarih'])){
                $sql .= "and  MBS.BASLANGIC_TARIHI <= TO_DATE(".$post['bitTarih'].", 'dd/mm/yyyy')";
                $sqlKur .= "and  MBS.BASLANGIC_TARIHI <= TO_DATE(".$post['bitTarih'].", 'dd/mm/yyyy')";
            }
            ...
            $sqlKur = "select DISTINCT MBS.KURULUS_ID
					 from m_belgelendirme_sinav MBS
					  join m_yeterlilik on MBS.yeterlilik_id = m_yeterlilik.yeterlilik_id
					where MBS.IPTAL_USER IS NOT NULL AND ((MBS.bildirim_durumu = 0 AND MBS.BASLANGIC_TARIHI<TO_DATE(sysdate, 'dd/mm/yyyy'))
					OR (MBS.GECERLILIK_DURUMU = 2)
					OR (MBS.bildirim_durumu=1 AND MBS.BASLANGIC_TARIHI<=TO_DATE(sysdate, 'dd/mm/yyyy') AND MBS.SINAV_ID NOT IN (SELECT DISTINCT SINAV_ID FROM M_BELGELENDIRME_SINAV_DOSYA)))";
            ...
            if(!empty($post['basTarih']) && !empty($post['bitTarih'])){
                $sql .= " and SG.BASLANGIC_TARIHI >= TO_DATE('".$post['basTarih']."', 'dd/mm/yyyy') and  SG.BASLANGIC_TARIHI <= TO_DATE('".$post['bitTarih']."', 'dd/mm/yyyy')";
                $sqlKur .= " and SG.BASLANGIC_TARIHI >= TO_DATE('".$post['basTarih']."', 'dd/mm/yyyy') and  SG.BASLANGIC_TARIHI <= TO_DATE('".$post['bitTarih']."', 'dd/mm/yyyy')";
            }
            else if(!empty($post['basTarih'])){
                $sql .= " and SG.BASLANGIC_TARIHI >= TO_DATE('".$post['basTarih']."', 'dd/mm/yyyy')";
                $sqlKur .= " and SG.BASLANGIC_TARIHI >= TO_DATE('".$post['basTarih']."', 'dd/mm/yyyy')";
            }
            else if(!empty($post['bitTarih'])){
                $sql .= "and  SG.BASLANGIC_TARIHI <= TO_DATE(".$post['bitTarih'].", 'dd/mm/yyyy')";
                $sqlKur .= "and  SG.BASLANGIC_TARIHI <= TO_DATE(".$post['bitTarih'].", 'dd/mm/yyyy')";
            }
