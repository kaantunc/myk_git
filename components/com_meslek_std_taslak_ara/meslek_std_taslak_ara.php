...
if($isProtokol){
	$baslik = "Protokol Kapsamındaki Meslek Standartları";
}
Else if($isGorus){
	$baslik = "Görüş Aşamasındaki Meslek Standartları";
}
...
function formGoster($itemIdStr, $protokolStr, $gorusStr){
	...
	?>
<form
	action="index.php?option=com_meslek_std_taslak_ara<?php echo $protokolStr?><?php echo $gorusStr?><?php echo $itemIdStr?>"
...
function hepsiIleListele($itemIdStrOrj, $isProtokol, $protokolStr, $isGorus, $gorusStr){
	...
	$sonuclar = hepsiIleAra($db, $standart_adi, $sektor_id, $seviye_id, $gorev_adi
			, $olusturan, $isProtokol, $protokolStr, $isGorus, $gorusStr);
...
<table cellspacing="0" class="paginate-10 sortable">
	...
	foreach($sonuclar AS $satir){
		...
		if($isGorus) echo '<td><a href="index.php?option=com_meslek_std_taslak&amp;view=gorus_bildir&amp;standartId='.$satir['STANDART_ID'].'">Görüs Bildir</a></td>';
...
<br />
<a
	href="index.php?option=com_meslek_std_taslak_ara<?php echo $protokolStr?><?php echo $gorusStr?><?php echo $itemIdStrOrj?>">Geri</a>
	<?php
}

function  hepsiIleAra($db, $standart_adi, $sektor_id, $seviye_id, $gorev_adi, $olusturan, $isProtokol, $isGorus){
	...
	if($isProtokol){
		/*$sql = "  SELECT 	
					...
					FROM M_MESLEK_STANDARTLARI 
                        JOIN M_MESLEK_STAN_EVRAK ON (M_MESLEK_STAN_EVRAK.STANDART_ID=M_MESLEK_STANDARTLARI.STANDART_ID) 
                        JOIN M_BASVURU ON (M_BASVURU.EVRAK_ID=M_MESLEK_STAN_EVRAK.EVRAK_ID)
                        JOIN M_KURULUS ON (M_KURULUS.USER_ID=M_BASVURU.USER_ID)
                        JOIN PM_SEVIYE ON (PM_SEVIYE.SEVIYE_ID=M_MESLEK_STANDARTLARI.SEVIYE_ID)
						" .
						(($gorevStr) ?
							" JOIN M_TASLAK_MESLEK ON (M_TASLAK_MESLEK.STANDART_ID=M_MESLEK_STANDARTLARI.STANDART_ID)
							  JOIN M_TASLAK_MESLEK_PROFIL ON (M_TASLAK_MESLEK.TASLAK_MESLEK_ID=M_TASLAK_MESLEK_PROFIL.TASLAK_MESLEK_ID)" : "").	
							"
						
						
						
            WHERE 
                        MESLEK_STANDART_SUREC_DURUM_ID<>-3 AND 
                        MS_LISTE_ONAY=1   AND BASVURU_DURUM_ID=6 	
			".($stdAdiStr ? ("AND ". $stdAdiStr) : '')."
			".($sektorIdStr ? ("AND ". $sektorIdStr) : '')."
			".($gorevStr ? ("AND ". $gorevStr) : '')." 
			".($olusturanStr ? ("AND ". $olusturanStr) : '')."
			".($seviyeIdStr ? ("AND PM_SEVIYE.". $seviyeIdStr) : '')."
		  ORDER BY KURULUS_ADI, STANDART_ADI";
		*/
		$sql = "  SELECT
		" .(($gorevStr) ? "	PROFIL_GOREV_ADI," : "")."
							STANDART_ID,
 ...
 JOIN M_KURULUS_YETKI USING (YETKI_ID)
		...
		(($gorevStr) ?
			"JOIN M_TASLAK_MESLEK_PROFIL USING (TASLAK_MESLEK_ID) " : "").	
			" 
		WHERE MESLEK_STANDART_SUREC_DURUM_ID = 4 	
		".($stdAdiStr ? ("AND ". $stdAdiStr) : '')."
		".($sektorIdStr ? ("AND ". $sektorIdStr) : '')."
		".($gorevStr ? ("AND ". $gorevStr) : '')." 
		".($olusturanStr ? ("AND ". $olusturanStr) : '')."
...
function generatePDFPathForStandart($standart_id)
{
	...
	if(count($data)>0)
	{
		if($data[0]['RESMI_GORUS_ONCESI_PDF']=='')//yani en ustteki revizyon
			// YOKSA PDF ÜRET
			$taslakUrl = 'index.php?option=com_meslek_std_taslak&amp;task=indir&amp;id=1&amp;standart_id='.$standart_id;
			//"index.php?option=com_meslek_std_taslak&amp;layout=pdf&amp;format=pdf&amp;form=5&amp;id=".getTaslakStandartEvrakId($_db, $standart_id)."&amp;standart_id=".$standart_id;
		else
