...
var LinkrAPI	= new Class({
	 ...
	 * Builds URL for JSON requests
	 */
	createListRequest : function(l, id, key) {
		return this.requestURL +'&request='+ l +'&'+ $pick(key, 'id') +'='+ id +'&__linkr='+ $time();
	},
	...
	getItemids : function(q, o, func, args)
	{
		...
		if ($type(q) == 'array' && q.length > 0)
		{
			...
			q.each(function(qe)
			{
				var a	= Linkr.dbQuote('%'+ qe);
				var b	= Linkr.dbQuote('%'+ qe +'&%');
				var c	= 'link LIKE '+ a +' OR link LIKE '+ b;
				qw.include('('+ c +')');
			});
			
			w.include('('+ qw.join(' AND ') +')');
		}
		...
		' WHERE '+ w.join(' AND ') +' ORDER BY ordering,name';
