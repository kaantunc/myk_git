<form
	...
	action="index.php?option=com_belgelendirme_basvur&amp;layout=ek&amp;task=belgelendirmeKaydet"
...
//EK Paneli
dPanels.personelForm_panel = new Array("EK: SINAV VE BELGELENDİRME EKİBİNDE GÖREVLENDİRİLEN PERSONELE İLİŞKİN BİLGİ FORMU",
		new Array("1. Kişisel Bilgiler", "baslik"),
		new Array("T.C. Kimlik No/ Pasaport No", "text"),
		new Array("Adı", "text"),
		...
		new Array("4. Sınav ve Belgelendirme sürecine ilişkin  alınan eğitimler/özel deneyimler", "baslik"),
		...
		new Array("7. Sınavlarda görev alacaklar için görev alacağı yeterlilikler", "baslik"),
		new Array("", "textarea", "", "5", "50"));
...
function createAddPersonelValues (name, buttonName){
	...
	for ($i=0; $i< $dataCount; $i++) {
		...
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_KIMLIK_NO"]) .'";';
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_ADI"]) .'";';
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_SOYAD"]) .'";';
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_TELEFON"]) .'";';
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_FAKS"]) .'";';
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_EPOSTA"]) .'";';
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_OGRENIM"]) .'";';
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_MESLEK"]) .'";';
		echo 'arry['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_UZMANLIK"]) .'";';
		
		echo 'deneyim['.$i.']= "'.FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_DENEYIM"]) .'";';
		echo 'yeterlilik['.$i.']= "'.FormFactory2::normalizeVariable ($arr["GOREVLI_PERSONEL_YETERLILIK"]) .'";';
	}
	...
	if (dataCount > 0){
		...
		addYeterlilikValues	(yeterlilik);
	}
}
...
function addYeterlilikValues (yeterlilik){
	var length = yeterlilik.length;

	for(var i = 1; i < length+1; i++){
		if (i == 1)
			inpName = "inputpersonelForm_panel-23-22";
		else
			inpName = "inputpersonelForm_panel"+ i +"-23-22";

		document.getElementById (inpName).value = yeterlilik[i-1];
	}
}
...
function addEgitimValues (egitim, panelCount){
	...
	if (isset ($data[0])){
		...
		for ($i=0; $i< $tableCount; $i++) {
			...
			echo 'arr['.$p.']['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["EGITIM_YERI"]) .'";';
		}
	}
	?>
...
function addSertifikaValues (sertifika, panelCount){
	...
	if (isset ($data[0])){
		...
		for ($i=0; $i< $tableCount; $i++) {
			...
			echo 'arr['.$p.']['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["SERTIFIKA_ADI"] ).'";';
		    echo 'arr['.$p.']['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["SERTIFIKA_YER"]) .'";';
		    echo 'arr['.$p.']['.$c++.']= "'. $arr["SERTIFIKA_TARIH"] .'";';
		    echo 'arr['.$p.']['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["SERTIFIKA_ACIKLAMA"]) .'";';
...
function addIsDeneyimValues (isDeneyim, panelCount){
	...
	if (isset ($data[0])){
		...
		for ($i=0; $i< $tableCount; $i++) {
		    ...
		    echo 'arr['.$p.']['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["DENEYIM_ISYERI"]) .'";';
		    echo 'arr['.$p.']['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["DENEYIM_UNVAN"]) .'";';
		    echo 'arr['.$p.']['.$c++.']= "'. FormFactory2::normalizeVariable ($arr["DENEYIM_ISTANIMI"]) .'";';
		}
	}
	?>
	...
	if (isset ($data[0])){
		...
		for ($i=0; $i< $tableCount; $i++) {
			...
			if ($personel_id != $arr ["GOREVLI_PERSONEL_ID"]){
				...
				echo "arr[".$p."] = new Array ();";
				echo 'arrId['.$id.']= "'. $arr["GOREVLI_PERSONEL_ID"] .'";';
				echo 'arr['.$p.'][0]= "'. FormFactory2::normalizeVariable ($arr["DIL_ADI"]) .'";';
				$personel_id = $arr ["GOREVLI_PERSONEL_ID"];
				$id++;
				$c=1;
			}
			...
			if ($c % 5 == 0){
				echo 'arr['.$p.']['.$c.']= "'. FormFactory2::normalizeVariable ($arr["DIL_ADI"]) .'";';
				$c++;
			}
