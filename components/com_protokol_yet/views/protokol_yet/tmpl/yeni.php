...
{
	if(!$this->yetProtokoluMu)
	{
		global $mainframe;
		$mainframe->redirect("index.php?option=com_protokol_yet&layout=yeni", JText::_("PROTOKOL_CHECK_ERROR"), 'error');
	}
	...
	$yetSayisi = $seciliProtokol[0]["SAYISI"];
...
$yeterlilikSeviyeleri = $this->yeterlilikSeviyeleri;
$yeterlilikSektorleri = $this->yeterlilikSektorleri;

//SEVIYELER
$seviyeOptions = "<option value=''></option>";
foreach($yeterlilikSeviyeleri as $seviye){
	$seviyeOptions .= "<option value='".$seviye["SEVIYE_ID"]."'>".$seviye["SEVIYE_ADI"]."</option>";
}

//SEKTORLER
$sektorOptions = "<option value=''></option>";
foreach($yeterlilikSektorleri as $sektor){
...
<div class="form_item">
	<div class="form_element cf_heading">
		<h1 class="contentheading">Yeterlilik Protokolü</h1>
	</div>
...
<div class="form_content">
	<form method="post" id="yeniprotokolForm"
		action="index.php?option=com_protokol_yet&amp;task=protokolKaydet">
		...
		<div class="form_row">
			<div class="<?php echo $classBirinci; ?>">
				<strong>Yeterlilik Sayısı:</strong>
			</div>
			<div class="<?php echo $classIkinci; ?>">
				<input type="text" name="yetSayisiTextbox" class="required"
					id="yetSayisi"  onkeyup="jQuery(this).val(jQuery(this).val().replace(/\D/g,''));"  value="<?php echo $yetSayisi; ?>" style="width: 35px;" />
			</div>
			<div style="clear: both;"></div>
		</div>
		<div class="form_row">
			...
			<div class="<?php echo $classIkinci; ?>">
				<select name="protokolSektorleriMultipleSelect[]"  id="protokolSektorleriMultipleSelect"  multiple="multiple">
					<?php
					$sektorler = $this->yeterlilikSektorleri;
				...
				<div>
					<table cellpadding="0" cellspacing="0" border="0" class="display"
						...
						for($i=0; $i<count($kuruluslar); $i++)
						{
							...
							$allTheRowContents .= '<td><input type="checkbox" value="'.$kuruluslar[$i]["USER_ID"].'" name="kurulusCheckbox[]" class="kurulusCheckbox" '.$checked.' ></td>';
...
jQuery(document).ready(function() {	
	...
	jQuery('#kaydetButton').live('mousedown', function (e) {
		...
		jQuery("#yeniprotokolForm").attr("action", "index.php?option=com_protokol_yet&amp;task=protokolKaydet" + kurulusIdleriText + kurulusRolleriText);
...
function addProtokolDosya (tableName){
	...
	if (path != null && path != ''){
		...
		result 	  += '<input type="button" value="İndir" onclick="window.location.href=\'index.php?option=com_protokol_yet&amp;task=indir&amp;protokolID=<?php echo $protokolID;?>\'" class="up_submitbtn" style="float:none;"> <\/div>';
...
function saveUzatmaRow ( oTable, nRow, isSave )
{
	...
	var url = 'index.php?option=com_protokol_yet&task=ajaxUzatmaKaydet&format=raw&protokolID='+protokolID;

	if (!isSave){
		url = 'index.php?option=com_protokol_yet&task=ajaxUzatmaGuncelle&format=raw&protokolID='+protokolID;
...
function deleteUzatmaRow ( oTable, nRow)
{
	...
	jQuery.ajax({
		  url: "index.php?option=com_protokol_yet&task=ajaxUzatmaSil&format=raw&protokolID="+protokolID,
		  data: sendData,
		  type: "POST",
		  dataType: 'json',
		  success: function(data) {
