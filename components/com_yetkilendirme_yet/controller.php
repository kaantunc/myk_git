...
class Yetkilendirme_YetController extends JController {
	...
	function protokolKaydet()
	{
		...
		if(strlen($protokolID)==0) // ADD
		{
			...
			$this->setRedirect('index.php?option=com_yetkilendirme_yet&layout=yeni&protokolID='.$protokolID, $message);
		}
		else // EDIT
		{
	...
	function protokolKaydet_YeniProtokolEkle()
	{
		...
		if($message)//SUCCESS, MUST EDIT
			$this->setRedirect('index.php?option=com_yetkilendirme_yet&layout=yeni&protokolID='.$protokolID, "Başarıyla Eklendi");
		else
	...
	function etkisizlestir()
	{
		...
		$this->setRedirect('index.php?option=com_yetkilendirme_yet', $message);
	}
