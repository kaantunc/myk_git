		...
		<div>
			<table id="protokolListe">
				...
				for($i=0; $i<count($protokoller); $i++)
				{
					...
					echo '<td><a href="index.php?option=com_yetkilendirme_yet&layout=yeni&protokolID='.$protokoller[$i]["YETKI_ID"].'">'.$protokoller[$i]["ADI"].'</a></td>';
...
function OnButton3()
{
	...
	{		
	    document.protokolForm.action = "index.php?option=com_yetkilendirme_yet&amp;task=sil";
	    document.protokolForm.submit();             // Submit the page
	}
    return true;
}
...
function OnButton5()
{
    document.protokolForm.action = "index.php?option=com_yetkilendirme_yet&amp;layout=yeni";
    document.protokolForm.submit();             // Submit the page
    return true;
}
