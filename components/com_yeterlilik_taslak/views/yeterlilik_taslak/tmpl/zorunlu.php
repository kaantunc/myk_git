	...
	<div class="form_element cf_placeholder">
		...
		<table id="tablo_zorunlu_birim" style="width: 100%;">
			...
			<tbody>
				<?php 
				$sqy = 1;
				foreach ($this->zorunluBirim as $row){
					if($sqy%2==0){
...
?>
	<div class="form_item" style="padding-top: 25px;">
		<div class="form_element cf_button">
			<input value="Yani Zorunlu Birim Ekle" id="zBirimekle" type="button" />
		</div>
		<div class="cfclear">&nbsp;</div>
	</div>
<?php 
...
<?php 
$zorunluBirimTur = $this->zorunluBirimTur;
foreach ($this->zorunluBirim as $row){?>
...
<form action="index.php?option=com_yeterlilik_taslak&task=turKaydet&layout=zorunlu&yeterlilik_id=<?php echo $this->yeterlilik_id;?>" method="post" style="padding:5px" id="turForm_<?php echo $row['YETERLILIK_ALT_BIRIM_ID'];?>">
<table id="birimTurleri_<?php echo $row['YETERLILIK_ALT_BIRIM_ID'];?>" border="1" style="width:100%; float:left;margin-bottom:10px;">
	...
	<?php 
	$say = 1;
	foreach($zorunluBirimTur[$row['YETERLILIK_ALT_BIRIM_ID']] as $cow){
		if($cow['TUR'] == 1){
...
<div id="zorunluBirimler" style=" min-width: 10px; min-height:10px; background-color: white; border:1px solid #00A7DE; display: none; padding:20px">
    <form id="zorunluForm" action="index.php?option=com_yeterlilik_taslak&layout=zorunlu&task=taslakKaydet&yeterlilik_id=<?php echo $this->yeterlilik_id;?>" method="post">
    <strong>Zorunlu Birim Adı</strong><br><br>
    <input type="text" name="zbirim" id="zbirim" size="50"/><br><br>
    <input type="hidden" name="tip" value="1"/>
    <input type="button" value="Kaydet" id="zorunluKaydet"/>
    <input type="button" value="İptal" id="zorunluIptal"/>
    </form>
</div>
...
// 	var headers	  = new Array ('Sıra No', 'Birim No','Birim Adı');
...
jQuery(document).ready(function(){
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				if(dat){
					...
					if(confirm('Zorunlu birimi silmek istediğinizden emin misiniz?')){
						jQuery('#zorunluBirimler').trigger('close');
	...
	jQuery('#zBirimekle').live('click',function(e){
		e.preventDefault();
		jQuery('#zbirim').val('');
		jQuery('#zorunluBirimler').lightbox_me({
            centered: true,
            closeClick:false,
            closeEsc:false
        });
	...
	jQuery('#zorunluIptal').live('click',function(e){
		e.preventDefault();
		jQuery('#zorunluBirimler').trigger('close');
	});

	jQuery('#zorunluKaydet').live('click',function(e){
		e.preventDefault();
		var birim = jQuery('#zbirim').val();
		if(birim = '' || birim.length == 0){
			alert('Lütfen Zorunlu Birim Adını boş bırakmayınız.');
		}
		else{
			jQuery('#zorunluForm').submit();
		}
	});
	...
	jQuery('#aciklamaKaydet').live('click',function(e){
		e.preventDefault();
		var acik = jQuery('#zorunlu_aciklama').val();
		var yetId = <?php echo $this->yeterlilik_id;?>;
		jQuery('#loaderGif').lightbox_me({
            centered: true,
            closeClick:false,
		...
		jQuery.ajax({
			...
			data:"acik="+acik+'&tur=zorunlu_birim&yetId='+yetId,
			success:function(data){
