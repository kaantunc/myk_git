...
function yetSecildi(){
	...
	xmlhttpPost(null, 'index.php?option=com_sinav_sonuc&format=raw', kapsamGetQueryString, kapsamUpdatePageFunction);
...
function kapsamGetQueryString(){
	...
	var qstr = 'yetId=' + yetId + '&gorev=kapsamAl';
	return qstr;
}
...
function insertIlkSayfa(){
	...
	if(seciliSayfa == 1)
		xmlhttpPost(checkfirstPage, 'index.php?option=com_sinav_sonuc&format=raw',firstPageGetQueryString,firstPageUpdatePageFunction);
	else
		goNextPage();
}
...
function firstPageGetQueryString(){
	...
	var qstr = 'gorev=ilkSayfa&evrakId=' + evrakId + '&userId=' +userId+ '&yeterlilikId=' + yeterlilikId + '&sinavTarihi=' + sinavTarihi + '&toplamAday=' + toplamAday + '&basariliAday=' + basariliAday;
	//alert(qstr);
	return qstr;
}
...
function firstPageUpdatePageFunction(str){
	...
	var t34Form = document.getElementById("ChronoContact_t34");
	...
	// document.getElementById( "ilcelerSil" ).innerHTML = str;
	// if("<?php echo isset($_GET['ilce']) ? "1" : "0" ?>" == "1"){

		// var selectobject=document.getElementById("selectIlcelerSil");
		// for (var i=0; i<selectobject.length; i++){
			// if(selectobject.options[i].value=="<?php echo isset($_GET['ilce']) ? $_GET['ilce'] : "" ?>"){
