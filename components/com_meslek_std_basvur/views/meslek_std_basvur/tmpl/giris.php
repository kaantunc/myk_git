...
<div class="form_item">
  <div class="form_element cf_heading">
  	<h3 class="contentheading">Meslek Standardı Başvuruları</h3>
  </div>
...
<div class="form_item">
	<div id="basvurular_div">
		<table id="basvurular" style="float:left; width:100%;text-align:center">
			...
			<?php
	                ...
	                if($basvurular[$i][DURUM_ID] == -1 || $basvurular[$i][DURUM_ID] == -2){
	                	echo '<td style="text-align:center;"><input type="button" id="'.$basvurular[$i][EVRAK_ID].'" class="gonder" value="İncele/Değiştir" onclick="goToNewPage('.$basvurular[$i][EVRAK_ID].')"/></td>';
	                }
	                else{
	                	echo '<td style="text-align:center;"><input type="button" id="'.$basvurular[$i][EVRAK_ID].'" class="gonder" value="İncele" onclick="goToNewPage('.$basvurular[$i][EVRAK_ID].')"/></td>';
	                }
		...
		<?php if($basSayi == 0){?>
		<input type="button" name="satirEkleButton"  class="satirEkle" value="Yeni Meslek Standardı Hazırlama Başvurusu Ekle" onclick="goToNewPage(-1)"/>
		<?php }
			else{
		?>
		<input type="button" name="satirEkleButton"  class="satirEkle" value="Yeni Meslek Standardı Hazırlama Başvurusu Ekle" onclick="goToNewPage(-1)"/>
...
function goToNewPage(evrakid){
	window.location.href="index.php?option=com_meslek_std_basvur&layout=basvuru_yeni&evrak_id="+evrakid;
}
...
function deleteBelge(evrakId){
	if(confirm('Başvuruyu silmek istediğinizden emin misiniz?')){
		...
		jQuery.ajax({
			type:"POST",
			url:"index.php?option=com_meslek_std_basvur&task=ajaxBasvuruSil&format=raw",
