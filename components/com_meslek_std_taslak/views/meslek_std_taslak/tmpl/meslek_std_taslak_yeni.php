	...
	function addTaslakPdf (tableName){
		...
		for (var i = 0; i < 5; i++){//6
			if (paths[i] != null && paths[i] != ''){
				...
				var result = inputPath + '<br><div id="success_'+tableName+'_0_'+sira+'" class="up_success">'+fileNames[i]+' yüklendi!</div><div> <input type="button" value="İndir / Oku" onclick="window.location.href=\'index.php?option=com_meslek_std_taslak_yeni&amp;view=taslak_revizyon&amp;task=indir&amp;id='+sira+'&amp;standart_id=<?php echo $this->standart_id;?>&amp;revize_no=<?php echo $_GET[revize_no];?>\'" class="up_submitbtn" style="float:none;"><input type="button" value="Sil / Değiştir" onclick="window.location.href=\'index.php?option=com_meslek_std_taslak_yeni&amp;view=taslak_revizyon&amp;task=sil&amp;id='+sira+'&amp;standart_id=<?php echo $this->standart_id;?>&amp;revize_no=<?php echo $revize_no;?>\'" class="up_submitbtn" style="float:none;"></div>';
